...
`generationId()` returns the nonblank generation captured during handoff preparation, before object conversion and validation. Publication can make a newer generation current before handoff returns; the admitted handle and every nested REST call still use the captured ID. Repeated and concurrent reads remain safe after execution, failure, release, retirement, or shutdown cutoff and do not claim or extend the admission. The ID is process-local, not a durable snapshot ID. Stage the application-owned generation-to-snapshot mapping before admitting traffic, record the association before invocation, and handle a missing mapping explicitly. Application retention may remove a mapping after ownership ends, including at cutoff, even while the handle still exposes its ID. `release()` in `finally` also covers lookup or recording failure and is harmless after execution claims the admission.
...
Register before staging the initial generation; the application controls its traffic gate. Stage each candidate's resources before publication, since retirement of an unused prior generation can be reported before `publish` returns. The callback identifies a superseded published generation only after captured preparation, pending admissions, roots, and physically running descendants have released it. It may run concurrently on publication, invocation, admission-release, or physical-return threads. Keep it thread-safe and prompt; schedule slow cleanup on an application executor. A listener failure is logged and cannot fail publication or invocation or prevent other listeners; the application owns cleanup retries. Closing `retirement` prevents future selection but does not wait for a callback already selected or running. Notifications have no replay, durable delivery, retry, or cross-generation ordering guarantee, and may be omitted once shutdown begins, including for the active generation. IDs are process-local. Clean up staged resources for a never-published rejected or discarded candidate yourself; a repeated publish rejection for an already published candidate is not a reason to discard its resources.

`prepare()` synchronously reads and validates the complete YAML set against fixed Java declarations, model connections, and handler beans, then freezes it without activation. The application must finish file publication and prevent writes until prepare returns; Loomspan does not provide a multi-file transaction. Changes or deletion after prepare do not alter that candidate. Every successful prepare gets a fresh process-local ID, even for identical YAML; an empty YAML set deletes all YAML skills while retaining Java declarations. `publish` does not reread files. It rejects foreign, stale, repeated, or shutdown-time candidates with `SkillReloadException`; failed preparation leaves the active generation unchanged. A stale candidate must be prepared afresh. `snapshot()` performs no filesystem reads, and old catalog objects stay unchanged. Publication itself is not a safe-deletion signal; the retirement callback provides that signal after final ownership release during normal operation. A retained admitted handle still reports its ID after retirement, but does not keep an application mapping available; the application chooses its correlation retention policy. Loomspan exposes no historical invocation API.

Applications that hold YAML in memory can prepare the complete replacement set with `reloader.prepare(Collection<SkillDocument>)`. Each `SkillDocument(sourceName, yaml)` has a nonblank, exact-unique source name and non-null YAML text. The source name is an arbitrary diagnostic label, not a file location or the callable skill name; YAML `name` remains the exact invocation identity. Preparation copies the collection, validates every document through the same manifest rules as configured files, and freezes its UTF-8 YAML. An empty collection removes all YAML and REST declarations while retaining fixed Java skills. A later no-argument `prepare()` still reads the configured locations. The overload does not read the supplied labels as paths or write files.

For editor feedback, use `reloader.validate()` to reread configured YAML or `reloader.validate(documents)` to check a complete in-memory replacement set. The latter never opens source labels as paths. An empty collection is a valid proposed removal of YAML and REST skills while retaining fixed Java skills. Check `SkillValidationResult.valid()` before using `skills()` as the complete candidate: any `ERROR` makes it false and leaves `skills()` empty, while warnings retain complete metadata. An empty list alone cannot distinguish an invalid result from a valid empty candidate. Each immutable `ValidatedSkill` contains an exact callable `name` and public `SkillKind`; successful results include fixed Java and all proposed model-backed YAML and REST skills, sorted by exact case-sensitive name. The result lists are immutable and detached from later operations. Issues carry a source name, optional skill name and field path, and a message. `<documents>` identifies malformed collection entries and `<configured-resources>` identifies discovery failures. Validation checks the same authoring rules as preparation, including model and schema configuration, Java-name and child conflicts, input contracts, and REST handler declarations. It returns schema-complexity warnings without editor-validation log noise. Repeated validation changes no generation, candidate, handler, cache, or retirement state. It does not run skills, test connectivity, construct a REST handler, or authorize publication. After editing, call `prepare()` or `prepare(documents)`, stage application resources using the prepared candidate's `snapshot()` as the authoritative description of that frozen candidate, then `publish` it. Preparation rereads and rechecks its own current input, so successful validation does not guarantee later preparation, resource staging, or publication.

On restart, the application can load its own durable `List<SkillDocument>`, let Loomspan start normally from configured locations, then call `prepare(savedDocuments)`, stage REST configuration under the new candidate ID, and publish before opening its traffic gate. Generation IDs are process-local; durable snapshots and readiness sequencing belong to the application. Loomspan intentionally has no startup loading switch, deferred activation mode, or framework traffic gate.

```java
List<SkillDocument> savedDocuments = snapshotStore.loadDocuments();
PreparedSkillUpdate restored = reloader.prepare(savedDocuments);
configurationStore.stage(restored.generationId(), snapshotStore.loadRestConfiguration());
reloader.publish(restored);
trafficGate.open();
```
...
Console displays the declared source as YAML, REST, or Java. YAML and REST details contain a descriptive source label and YAML text; Java details contain bean and declared method. File-derived labels are paths; application-supplied labels are arbitrary text. Links and pagination use the registered skill name.
