...
- the simplest coherent solution that fully accomplishes the committed goal;
- reuse and extension of existing framework concepts and functionality before new parallel concepts are introduced;
- active minimization of technical debt, including removal rather than preservation of obsolete internal paths;
...
### 1. Prefer the simplest complete design and reuse what exists

Choose the simplest path that fully accomplishes the feature's goal and preserves its required production invariants. Simplicity does not mean implementing only an easy subset, deferring essential correctness, or choosing the smallest immediate patch. It means avoiding concepts, layers, state, branches, and machinery that are not necessary for the complete result.

Before inventing a new abstraction, state model, lifecycle, identifier, extension point, or parallel execution path, inventory the framework's existing concepts and functionality. Prefer extending, composing, narrowing, or relocating an existing mechanism when it already owns substantially the same responsibility. Introduce a new concept only when the existing model cannot express the required behavior clearly and safely, and make that gap explicit.

Minimize technical debt in both the destination design and the transition. Avoid duplicate authorities, parallel implementations of the same rule, unused payload fields, speculative generalization, compatibility machinery without a protected contract, and temporary paths with no concrete removal plan. The simplest coherent final state matters more than preserving an internal decomposition merely because it already exists.

Research and ticket planning are also opportunities to evaluate the code and concepts encountered. If investigation finds dead code, an unreachable branch, an unused abstraction or parameter, duplicated functionality, or a concept that appears to provide little framework value, flag it explicitly for the developer with source evidence and a recommendation. Do not silently design around it or preserve it as an unquestioned constraint. Remove it within the proposed work when that removal is clearly in scope; otherwise bring it to the developer's attention for a deliberate decision rather than expanding the ticket without agreement.

### 2. Design for capable production models
...
### 3. Optimize for understandable developer experiences
...
### 4. Keep ordinary business input explicit
...
### 5. Separate business input from trusted execution metadata
...
### 6. Prefer explicit dataflow over ambient behavior
...
### 7. Default run-scoped state to immutability
...
### 8. Preserve production boundaries
...
### 9. Prefer visible failure over silent repair
...
### 10. Require evidence proportional to the abstraction
...
### 11. Account for the long-term cost of helpfulness
...
### 12. Keep semantic authority at the owning boundary

When the framework makes a runtime decision or owns a fact that cannot be recovered without interpreting the consequences of that decision, record the fact explicitly at its authoritative lifecycle boundary. Downstream components must not reconstruct it from record adjacency, completion order, timing, status differences, route names, thread identity, or other incidental effects.

A downstream reader may validate an authoritative fact against resulting state, but it must not maintain an independent classifier that can assign different semantic meaning.

This does not require the framework to materialize every computable projection. Console and other consumers should continue deriving aggregates, correlations, indexes, and presentation facts when those are genuinely new analytical concepts computed from complete canonical inputs rather than reconstructions of a decision the framework already made.

Before choosing derivation over emission, identify:

- which component owns the fact or decision;
- whether the downstream result is reconstruction or new analysis;
- whether every valid execution, including failure before later lifecycle records, retains enough canonical information; and
- whether emission would create a duplicate authority or instead eliminate one.
...
- Does a downstream consumer have to infer a semantic decision that the runtime already made authoritatively?
- Could a clearer contract, error, trace, sample, or prompt resolve the issue without a new abstraction?

### Simplicity, reuse, and code health

- What is the simplest coherent solution that fully accomplishes the goal?
- Which existing framework concepts, owners, lifecycle mechanisms, validators, paths, or utilities can be reused or narrowed?
- Does the proposal introduce a second representation, authority, abstraction, or execution path for functionality the framework already has?
- What technical debt does the proposal remove, retain, or create?
- Are any proposed fields, parameters, identifiers, interfaces, beans, compatibility paths, or intermediate values derivable, unused, or speculative?
- Did source research uncover dead code, unreachable branches, duplicated functionality, or low-value concepts that should be removed or brought to the developer's attention?
...
13. the existing framework concepts and functionality investigated for reuse, what will be reused or simplified, and why any genuinely new concept is necessary;
14. technical-debt effects and any dead, duplicated, unused, unreachable, or low-value code or concepts discovered during research, including whether removal is in scope or requires a separate developer decision.
