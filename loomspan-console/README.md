...
- `LOOMSPAN_query_trace_plans` for whole authoritative plans in creation and
  accepted task/execution-unit order;
...
Ordinary MCP `tools/list` is the inventory for these thirteen tools.
`LOOMSPAN_get_runtime` reports current Console/target status and deliberately
contains no capability inventory. Installed tool availability remains distinct
from target authentication, compatibility, live availability, and evidence
state.
...
inventory/plan/frame/record/search calls, omitted `pageSize` defaults to 64 and
the value is a maximum: Console may stop
...
The twelve evidence-inspection tools return exactly one structured envelope arm:
...
remain SDK or HTTP failures. `LOOMSPAN_get_runtime` returns its status object
directly instead. Every success also includes one deterministic text
...
The finalized-trace workflow is `list traces -> get trace -> query whole plans
-> compact frames -> record descriptors/search -> selected content read`.
Omitted inventory order defaults to `FINALIZED_DESC`; frame order/projection
default to `CANONICAL`/`COMPACT`; record representation defaults to `LOGICAL`,
and inline content is opt-in.
Inventory supports source,
...
Frame queries default to `COMPACT`, which contains structural orientation only
and omits duration, assignment, usage, identity, and outcome detail.
`DETAILED` adds elapsed and self-duration, assignment, and rich
usage/retry/validation/failure/gap/uncertainty facts. A detailed frame has an
optional scalar close `outcome`; no close status means it is absent. Frame
filters compose with AND semantics, including
...
separately configured protected local MCP connection described above. It uses
ordinary MCP tool discovery for the thirteen-tool inventory, then
`LOOMSPAN_get_runtime` for current status. Missing a tool stops only dependent
work and is distinct from protocol, target compatibility, authentication,
evidence, and scope errors. Missing raw inspection removes only exact
storage/parser/projector forensics. Without MCP, the skill can explain practice but must report live
...
route filters the current summary, complete active branches, and follow/pause
