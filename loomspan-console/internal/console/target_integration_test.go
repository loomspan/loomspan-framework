...
func TestApplicationCredentialNeverAppearsOutsideSelectedRequestHeader(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		_, _ = response.Write([]byte(`{"instanceId":"11111111-1111-4111-8111-111111111111","consoleCompatibilityVersion":"1.0.0-beta.3","observedAt":"2026-07-25T12:00:00Z","liveMonitoringAvailable":true}`))
	}))
	defer server.Close()
...
func TestTargetContextNotifiesScopeOwnersOnActivationAndRotation(t *testing.T) {
	...
	server2 := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		_, _ = response.Write([]byte(`{"instanceId":"22222222-2222-4222-8222-222222222222","consoleCompatibilityVersion":"1.0.0-beta.3","observedAt":"2026-07-25T12:00:00Z","liveMonitoringAvailable":true}`))
	}))
	defer server2.Close()
...
func TestArtifactServiceAcquiresAndUsesThroughTargetScope(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		if strings.HasSuffix(request.URL.Path, "/instance") {
			_, _ = response.Write([]byte(`{"instanceId":"11111111-1111-4111-8111-111111111111","consoleCompatibilityVersion":"1.0.0-beta.3","observedAt":"2026-07-25T12:00:00Z","liveMonitoringAvailable":true,"registeredSkillCount":1,"activeExecutionCount":0,"catalogedTraceCount":0,"tracePersistencePolicy":"PERSISTENT","completionGraceTtl":"PT2M","traceCatalogMetadataTtl":"PT168H"}`))
			return
		}
...
func TestArtifactScopeRotationDuringMetadataFetchReturnsTargetChanged(t *testing.T) {
	...
	server2 := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		response.Header().Set(applicationclient.InstanceIDHeader, "22222222-2222-4222-8222-222222222222")
		_, _ = response.Write([]byte(`{"instanceId":"22222222-2222-4222-8222-222222222222","consoleCompatibilityVersion":"1.0.0-beta.3","observedAt":"2026-07-25T12:00:00Z","liveMonitoringAvailable":true,"registeredSkillCount":1,"activeExecutionCount":0,"catalogedTraceCount":0,"tracePersistencePolicy":"PERSISTENT","completionGraceTtl":"PT2M","traceCatalogMetadataTtl":"PT168H"}`))
	}))
	defer server2.Close()
	if err := targetContext.Select(server2.URL); err != nil {
		t.Fatal(err)
