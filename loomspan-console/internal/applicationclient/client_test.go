...
func TestInstanceRequestSendsExactlyOneBoundedCredentialHeader(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		_, _ = response.Write(validInstanceBody("1.0.0-beta.1"))
	}))
	defer server.Close()
	address, _ := NormalizeAddress(server.URL)
	client, err := New(address, testPolicy(), "1.0.0-beta.1")
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
...
func TestClientConsumesCommittedInstanceFixtureOnlyAfterExactCompatibility(t *testing.T) {
	...
	instance := serveBody(t, fixture, "1.0.0-beta.1")
	...
	failure := serveFailure(t, mismatch, "1.0.0-beta.1")
...
func TestTransportUsesOptionalCustomCAWithoutWeakeningVerification(t *testing.T) {
	...
	defer server.Close()
	address, _ := NormalizeAddress(server.URL)
	untrusted, _ := New(address, testPolicy(), "1.0.0-beta.1")
	...
	trusted, err := New(address, policy, "1.0.0-beta.1")
	if err != nil {
		t.Fatal(err)
	}
	defer trusted.Close()
	...
	wrongHost, _ := New(wrongAddress, policy, "1.0.0-beta.1")
