...
func TestInstanceRequestSendsExactlyOneBoundedCredentialHeader(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		_, _ = response.Write(validInstanceBody("1.0.0-beta.6-SNAPSHOT"))
	}))
	defer server.Close()
	address, _ := NormalizeAddress(server.URL)
	client, err := New(address, testPolicy(), "1.0.0-beta.6-SNAPSHOT")
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
...
func TestClientConsumesCommittedInstanceFixtureOnlyAfterExactCompatibility(t *testing.T) {
	...
	instance := serveBody(t, fixture, "1.0.0-beta.6-SNAPSHOT")
	...
	failure := serveFailure(t, mismatch, "1.0.0-beta.6-SNAPSHOT")
...
func TestTransportUsesDirectSelectedAuthorityAndNeverFollowsRedirect(t *testing.T) {
	...
	client, _ := New(address, testPolicy(), "1.0.0-beta.6-SNAPSHOT")
	defer client.Close()
...
func TestTransportUsesOptionalCustomCAWithoutWeakeningVerification(t *testing.T) {
	...
	defer server.Close()
	address, _ := NormalizeAddress(server.URL)
	untrusted, _ := New(address, testPolicy(), "1.0.0-beta.6-SNAPSHOT")
	...
	trusted, err := New(address, policy, "1.0.0-beta.6-SNAPSHOT")
	if err != nil {
		t.Fatal(err)
	}
	defer trusted.Close()
	...
	wrongHost, _ := New(wrongAddress, policy, "1.0.0-beta.6-SNAPSHOT")
