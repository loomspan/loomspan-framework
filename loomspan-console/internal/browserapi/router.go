...
type TraceAnalysisService interface {
	...
	QueryPlans(context.Context, evidence.Reference, traceanalysis.PlanQuery) (traceanalysis.Page[traceanalysis.PlanSummary], *consolecore.Error)
...
func (router *Router) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	case "/api/console/v1/traces/analysis/plans":
		router.withSession(response, request, false, router.traceAnalysisPlans)
