...
type fakeTraceAnalysisService struct {
	...
	planScope         evidence.Reference
	planQuery         traceanalysis.PlanQuery
	planPage          traceanalysis.Page[traceanalysis.PlanSummary]
	planErr           *consolecore.Error
	planHook          func(context.Context)
	planContextErr    error
...
func (f *fakeTraceAnalysisService) QueryPlans(ctx context.Context, scope evidence.Reference, q traceanalysis.PlanQuery) (traceanalysis.Page[traceanalysis.PlanSummary], *consolecore.Error) {
	f.planScope = scope
	f.planQuery = q
	f.planContextErr = ctx.Err()
	if f.planHook != nil {
		f.planHook(ctx)
	}
	if f.planErr != nil {
		return traceanalysis.Page[traceanalysis.PlanSummary]{}, f.planErr
	}
	if q.Admit != nil {
		items := make([]traceanalysis.PlanSummary, 0, len(f.planPage.Items))
		for _, item := range f.planPage.Items {
			if !q.Admit(item) {
				if len(items) == 0 {
					return traceanalysis.Page[traceanalysis.PlanSummary]{}, consolecore.NewError(consolecore.CodeLimitExceeded, "One plan exceeds the safe response budget. Narrow the plan selector or increase the response budget.", scope.ID(), consolecore.Details{}, nil)
				}
				return traceanalysis.Page[traceanalysis.PlanSummary]{Items: items, HasMore: true, NextCursor: "admission-next"}, nil
			}
...
func (f *fakeTraceAnalysisService) QueryRecords(_ context.Context, scope evidence.Reference, q traceanalysis.RecordQuery) (traceanalysis.Page[traceanalysis.RecordSummary], *consolecore.Error) {
	if f.recordPage.Items != nil {
		if q.Admit != nil {
			for i, item := range f.recordPage.Items {
				if !q.Admit(item) {
					if i == 0 {
						return traceanalysis.Page[traceanalysis.RecordSummary]{}, consolecore.NewError(consolecore.CodeLimitExceeded, "Record exceeds response budget.", scope.ID(), consolecore.Details{}, nil)
					}
					return traceanalysis.Page[traceanalysis.RecordSummary]{Items: f.recordPage.Items[:i], HasMore: true, NextCursor: "record-next"}, nil
				}
			}
		}
...
func TestTraceAnalysisPlansReturnsBoundedAuthoritativeProjection(t *testing.T) {
	router, _, cookie, fake := traceAnalysisRouter(t)
	group, effective, overlap := "batch", true, false
	fake.planPage = traceanalysis.Page[traceanalysis.PlanSummary]{Items: []traceanalysis.PlanSummary{{
		PlanID: "plan-1", CapabilityName: `planner</script>`, CreatedAt: "2026-08-30T00:00:00Z", Status: "IN_PROGRESS",
		CreationSequence: 7, TraceRootFrameID: "root", MissionFrameID: "mission", PlanningFrameID: "planning",
		Tasks:          []traceanalysis.PlanTaskSummary{{StepNumber: 1, TaskID: "task-1", Title: `<img src=x>`, Status: "IN_PROGRESS", DependsOn: []string{}, ExpectedOutputs: []string{"answer"}, ParallelGroup: &group, EffectiveConcurrency: &effective, FailureIDs: []string{}}},
		ExecutionUnits: []traceanalysis.PlanExecutionUnitSummary{{Position: 0, ParallelGroup: &group, TaskIDs: []string{"task-1"}, EffectiveConcurrency: &effective, ObservedOverlap: &overlap}},
		Transitions:    []traceanalysis.PlanTransitionSummary{{Sequence: 8, Kind: "ADMISSION", TaskIDs: []string{"task-1"}, ParallelGroup: &group, EffectiveConcurrency: &effective}},
	}}, HasMore: true, NextCursor: "next-plan"}
	w := traceAnalysisRequest(router, "/api/console/v1/traces/analysis/plans", `{"source":"TARGET","traceId":"trace-1","planId":"plan-1","pageSize":2,"cursor":"cursor-1"}`, cookie)
	if w.Code != http.StatusOK {
		t.Fatalf("status=%d body=%s", w.Code, w.Body.String())
	}
	if fake.planScope != evidence.ForTarget(target.ScopeID("scope-1")) || fake.planQuery.Handle != artifact.Handle("opaque-handle") || fake.planQuery.PlanID != "plan-1" || fake.planQuery.PageSize != 2 || fake.planQuery.Cursor != "cursor-1" || fake.planQuery.Admit == nil {
		t.Fatalf("scope=%s query=%+v", fake.planScope, fake.planQuery)
	}
	for _, expected := range []string{`"planId":"plan-1"`, `"capabilityName":"planner\u003c/script\u003e"`, `"title":"\u003cimg src=x\u003e"`, `"parallelGroup":"batch"`, `"effectiveConcurrency":true`, `"observedOverlap":false`, `"kind":"ADMISSION"`, `"creationSequence":7`, `"nextCursor":"next-plan"`} {
		if !strings.Contains(w.Body.String(), expected) {
			t.Fatalf("missing %s in %s", expected, w.Body.String())
		}
	}
	if strings.Contains(w.Body.String(), "opaque-handle") || len(w.Body.Bytes()) > traceanalysis.MaxDescriptorResponseBytes {
		t.Fatalf("unsafe plan response bytes=%d body=%s", len(w.Body.Bytes()), w.Body.String())
	}
}

func TestTraceAnalysisPlansPreserveImportedOwnershipAndSanitizeErrors(t *testing.T) {
	router, _, cookie, fake := traceAnalysisRouter(t)
	fake.planPage = traceanalysis.Page[traceanalysis.PlanSummary]{Items: []traceanalysis.PlanSummary{}}
	w := traceAnalysisRequest(router, "/api/console/v1/traces/analysis/plans", `{"source":"IMPORTED","traceId":"trace-1"}`, cookie)
	if w.Code != http.StatusOK || fake.planScope != evidence.ForImported() || strings.Contains(w.Body.String(), "targetScopeId") {
		t.Fatalf("imported status=%d scope=%s body=%s", w.Code, fake.planScope, w.Body.String())
	}
	fake.planErr = consolecore.NewError(consolecore.CodeNotFound, "missing", "private-import-owner", consolecore.Details{}, nil)
	w = traceAnalysisRequest(router, "/api/console/v1/traces/analysis/plans", `{"source":"IMPORTED","traceId":"trace-1"}`, cookie)
	if w.Code != http.StatusNotFound || strings.Contains(w.Body.String(), "private-import-owner") || strings.Contains(w.Body.String(), "targetScopeId") {
		t.Fatalf("imported error status=%d body=%s", w.Code, w.Body.String())
	}
}

func TestTraceAnalysisPlanAdmissionExactFitOversizeAndContinuation(t *testing.T) {
	ref := evidence.ForTarget(target.ScopeID("scope-1"))
	base := traceanalysis.PlanSummary{PlanID: "plan", Tasks: []traceanalysis.PlanTaskSummary{}, ExecutionUnits: []traceanalysis.PlanExecutionUnitSummary{}, Transitions: []traceanalysis.PlanTransitionSummary{}}
	fits := func(size int) bool {
		admit := newPlanPageAdmission(ref)
		item := base
		item.CapabilityName = strings.Repeat("x", size)
		return admit(item)
	}
	low, high := 0, traceanalysis.MaxDescriptorResponseBytes
	for low < high {
	...
	if !fits(low) || fits(low+1) {
		t.Fatalf("admission boundary was not exact at %d", low)
	}

	router, _, cookie, fake := traceAnalysisRouter(t)
	oversized := base
	oversized.CapabilityName = strings.Repeat("x", low+1)
	fake.planPage = traceanalysis.Page[traceanalysis.PlanSummary]{Items: []traceanalysis.PlanSummary{oversized}}
	w := traceAnalysisRequest(router, "/api/console/v1/traces/analysis/plans", `{"source":"TARGET","traceId":"trace-1","pageSize":10}`, cookie)
	if w.Code != http.StatusTooManyRequests || !strings.Contains(w.Body.String(), `"code":"LIMIT_EXCEEDED"`) || !strings.Contains(w.Body.String(), "One plan exceeds the safe response budget") {
		t.Fatalf("oversized status=%d body=%s", w.Code, w.Body.String())
	}

	first := base
	first.CapabilityName = "small"
	fake.planPage = traceanalysis.Page[traceanalysis.PlanSummary]{Items: []traceanalysis.PlanSummary{first, oversized}}
	w = traceAnalysisRequest(router, "/api/console/v1/traces/analysis/plans", `{"source":"TARGET","traceId":"trace-1","pageSize":10}`, cookie)
	if w.Code != http.StatusOK || !strings.Contains(w.Body.String(), `"nextCursor":"admission-next"`) || strings.Count(w.Body.String(), `"planId":"plan"`) != 1 || len(w.Body.Bytes()) > traceanalysis.MaxDescriptorResponseBytes {
		t.Fatalf("continuation status=%d bytes=%d body=%s", w.Code, len(w.Body.Bytes()), w.Body.String())
	}
}

func TestTraceAnalysisPlansRequireSessionRejectMalformedAndNeedNoCSRF(t *testing.T) {
	router, _, cookie, _ := traceAnalysisRouter(t)
	valid := traceAnalysisRequest(router, "/api/console/v1/traces/analysis/plans", `{"source":"TARGET","traceId":"trace-1"}`, cookie)
	if valid.Code != http.StatusOK {
		t.Fatalf("no-CSRF status=%d body=%s", valid.Code, valid.Body.String())
	}
	unauthorized := traceAnalysisRequest(router, "/api/console/v1/traces/analysis/plans", strings.Repeat(" ", maxTraceAnalysisJSONBody+1), nil)
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("authentication did not precede body processing: %d", unauthorized.Code)
	}
	for _, body := range []string{`{"source":"TARGET","traceId":"trace-1","unknown":true}`, `{"source":"TARGET","traceId":"trace-1"}{}`, strings.Repeat(" ", maxTraceAnalysisJSONBody+1)} {
		bad := traceAnalysisRequest(router, "/api/console/v1/traces/analysis/plans", body, cookie)
...
func TestTraceAnalysisPlansPreserveArtifactExpiryCancellationAndTargetRotation(t *testing.T) {
	...
	expired := traceAnalysisRequest(expiredRouter, "/api/console/v1/traces/analysis/plans", `{"source":"TARGET","traceId":"trace-1"}`, expiredCookie)
	...
	cancel()
	request := httptest.NewRequest(http.MethodPost, "http://127.0.0.1:7943/api/console/v1/traces/analysis/plans", strings.NewReader(`{"source":"TARGET","traceId":"trace-1"}`)).WithContext(ctx)
	request.Host = "127.0.0.1:7943"
	request.Header.Set("Origin", "http://127.0.0.1:7943")
	request.AddCookie(cookie)
	router.ServeHTTP(httptest.NewRecorder(), request)
	if !errors.Is(fake.planContextErr, context.Canceled) {
		t.Fatalf("plan query did not receive canceled request context: %v", fake.planContextErr)
	}

	ids := []target.ScopeID{"scope-1", "scope-2"}
	rotatingTarget, err := target.New(func(applicationclient.Address) (target.ProbeClient, error) {
	...
	if domain := rotatingTarget.Select("http://127.0.0.1:8080"); domain != nil {
		t.Fatal(domain)
	}
	router.options.Target = rotatingTarget
	fake.planContextErr = nil
	fake.planHook = func(context.Context) {
		if domain := rotatingTarget.Select("http://127.0.0.1:8081"); domain != nil {
			t.Errorf("rotate target: %v", domain)
		}
	}
	rotated := traceAnalysisRequest(router, "/api/console/v1/traces/analysis/plans", `{"source":"TARGET","traceId":"trace-1"}`, cookie)
	if rotated.Code != http.StatusConflict || !strings.Contains(rotated.Body.String(), `"TARGET_CHANGED"`) {
		t.Fatalf("rotated status=%d body=%s", rotated.Code, rotated.Body.String())
	}
}
...
func TestTraceAnalysisFramesPreserveTimingUsageAndUnknownValues(t *testing.T) {
	...
	fake.framePage = traceanalysis.Page[traceanalysis.FrameSummary]{Items: []traceanalysis.FrameSummary{{
		...
		PlanID: stringPointer("plan-1"), TaskID: stringPointer("task-2"), StepNumber: &step, ParallelGroup: &group, EffectiveConcurrency: &effective,
	...
	}, {FrameID: "unassigned", ChildFrameIDs: []string{}, FrameType: "MODEL_CALL", Route: "unassigned", SkillNames: []string{}, AttemptIDs: []string{}, RetrySequenceIDs: []string{}, ValidationStatuses: []string{}, FailureIDs: []string{}, GapKinds: []string{}, UncertaintyKinds: []string{}}}}
	...
	for _, expected := range []string{`"planId":"plan-1"`, `"taskId":"task-2"`, `"stepNumber":2`, `"parallelGroup":"batch"`, `"effectiveConcurrency":true`, `"openedTimestampMillis":100`, `"closedTimestampMillis":112`, `"selfDurationMillis":null`, `"directUsage":{"promptUnits":3,"completionUnits":2,"totalUnits":5}`, `"directUsageComplete":false`, `"skillNames":["registered.skill"]`, `"attemptIds":["attempt-1"]`, `"failureIds":["failure-1"]`} {
		if !strings.Contains(w.Body.String(), expected) {
			t.Errorf("missing %s in %s", expected, w.Body.String())
		}
	}
	if !strings.Contains(w.Body.String(), `"route":"unassigned","planId":null,"taskId":null,"stepNumber":null,"parallelGroup":null,"effectiveConcurrency":null`) {
		t.Fatalf("unassigned frame did not preserve explicit nulls: %s", w.Body.String())
	}
}
...
func TestTraceAnalysisCompactFramesRetainHierarchyAndCountsOnly(t *testing.T) {
	...
	for _, forbidden := range []string{`"planId"`, `"taskId"`, `"stepNumber"`, `"parallelGroup"`, `"effectiveConcurrency"`, `"selfDurationMillis"`, `"directUsage"`, `"skillNames"`, `"attemptIds"`, `"failureIds"`} {
...
func TestTraceAnalysisRecordsExposeFailureIdentityAndRecordLocalValidationStatus(t *testing.T) {
	router, _, cookie, fake := traceAnalysisRouter(t)
	fake.recordPage = traceanalysis.Page[traceanalysis.RecordSummary]{Items: []traceanalysis.RecordSummary{
		{Sequence: 12, Type: "STEP_FAILED", FailureID: "failure-step", ValidationStatus: "exhausted"},
		{Sequence: 13, Type: "STRUCTURED_OUTPUT_RECORDED"},
	}}
	response := traceAnalysisRequest(router, "/api/console/v1/traces/analysis/records", `{"source":"TARGET","traceId":"trace-1","pageSize":1,"representation":"LOGICAL"}`, cookie)
	if response.Code != http.StatusOK || !strings.Contains(response.Body.String(), `"failureId":"failure-step"`) || !strings.Contains(response.Body.String(), `"validationStatus":"exhausted"`) {
		t.Fatalf("response=%d %s", response.Code, response.Body.String())
	}
	if strings.Count(response.Body.String(), `"validationStatus"`) != 1 {
		t.Fatalf("absent status was not omitted: %s", response.Body.String())
	}
}

func TestTraceAnalysisRecordsExposePlanIdentityAndCapability(t *testing.T) {
	router, _, cookie, fake := traceAnalysisRouter(t)
	fake.recordPage = traceanalysis.Page[traceanalysis.RecordSummary]{Items: []traceanalysis.RecordSummary{{
		Sequence: 12, Type: "PLAN_UPDATED", Facts: traceanalysis.RecordFacts{Plan: &traceanalysis.PlanReference{PlanID: "plan-1", CapabilityName: "investigateNetwork"}},
	}}}
	response := traceAnalysisRequest(router, "/api/console/v1/traces/analysis/records", `{"source":"TARGET","traceId":"trace-1","pageSize":1,"representation":"LOGICAL"}`, cookie)
	if response.Code != http.StatusOK || !strings.Contains(response.Body.String(), `"plan":{"planId":"plan-1","capabilityName":"investigateNetwork"}`) {
		t.Fatalf("response=%d %s", response.Code, response.Body.String())
	}
	for _, forbidden := range []string{`"traceRootFrameId"`, `"missionFrameId"`, `"planningFrameId"`, `"attemptId"`, `"retrySequenceId"`} {
		if strings.Contains(response.Body.String(), forbidden) {
			t.Fatalf("record plan reference retained %s: %s", forbidden, response.Body.String())
		}
	}
}
