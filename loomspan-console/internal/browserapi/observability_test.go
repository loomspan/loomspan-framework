...
func (client *fixtureObservabilityClient) Get(_ context.Context, endpoint string, _ int64, _ applicationclient.Credential) ([]byte, string, error) {
	...
	case strings.HasSuffix(endpoint, "/instance"):
		body = `{"instanceId":"` + instanceID + `","consoleCompatibilityVersion":"1.0.0-beta.6-SNAPSHOT","observedAt":"2026-07-27T00:00:00Z","liveMonitoringAvailable":true,"registeredSkillCount":1,"activeExecutionCount":1,"catalogedTraceCount":1,"tracePersistencePolicy":"PERSISTENT","completionGraceTtl":"PT2M","traceCatalogMetadataTtl":"PT168H"}`
	...
	case strings.Contains(endpoint, "/skills"):
		body = `{"items":[{"registeredName":"CheckDns","source": "YAML", "sourcePath":"classpath:/skills/check-dns.yaml"}],"hasMore":false,"nextCursor":null,"observedAt":"2026-07-27T00:00:00Z"}`
	case strings.Contains(endpoint, "/active-executions/"):
		body = client.activeExecutionBody()
	case strings.Contains(endpoint, "/active-executions"):
		body = `{"items":[` + client.activeExecutionBody() + `],"hasMore":false,"nextCursor":null,"observedAt":"2026-07-27T00:00:00Z","resumeCursor":"9"}`
...
func (client *fixtureObservabilityClient) activeExecutionBody() string {
	if client.activeBody != "" {
		return client.activeBody
	}
	return activeExecutionFixture()
}
...
func TestBrowserActiveExecutionFixturesPreserveAllBranchesAndNulls(t *testing.T) {
	for _, name := range []string{"empty.json", "serial-assigned.json", "grouped-disabled.json", "concurrent-siblings.json", "nested-inheritance.json"} {
		t.Run(name, func(t *testing.T) {
			content, err := os.ReadFile(filepath.Join("..", "..", "..", "loomspan-console-fixtures", "application-rest", "active-executions", "valid", name))
			if err != nil {
				t.Fatal(err)
			}
			client := &fixtureObservabilityClient{activeBody: strings.TrimSpace(string(content))}
			router, cookie := observabilityFixtureRouter(t, client)
			for _, request := range []struct{ path, body string }{{"/api/console/v1/active-executions/list", `{"pageSize":10}`}, {"/api/console/v1/active-executions/detail", `{"sessionId":"session-live"}`}} {
				response := apiRequest(router, request.path, request.body, cookie)
				if response.Code != http.StatusOK {
					t.Fatalf("%s status=%d body=%s", request.path, response.Code, response.Body.String())
				}
				body := response.Body.String()
				if !strings.Contains(body, `"activeBranches"`) {
					t.Fatalf("%s omitted active branches: %s", request.path, body)
				}
				for _, forbidden := range []string{"active" + "Path", "totalFrame" + "Depth", "active" + "Path" + "Truncated"} {
					if strings.Contains(body, forbidden) {
						t.Fatalf("%s retained %s: %s", request.path, forbidden, body)
					}
				}
			}
		})
...
func observabilityFixtureRouter(t *testing.T, client *fixtureObservabilityClient) (*Router, *http.Cookie) {
	t.Helper()
	...
	router, err := New(Options{Policy: policy, Pairing: pairing, Sessions: registry, PairingURL: func(value string) string { return value }, Target: targetContext, Observability: observability.New()})
	if err != nil {
		t.Fatal(err)
	}
	return router, browserauth.SessionCookie(sessionID)
}
