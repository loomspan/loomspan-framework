...
func (processor *Processor) Process(req artifact.ProcessRequest) (result artifact.ProcessResult, domain *consolecore.Error) {
	...
	assignmentResolver, d := plans.assignmentResolver(frames, scopeID)
	if d != nil {
		return artifact.ProcessResult{}, d
	}
	frameResults, gaps, uncertainties, ok := frames.resultsWithResolver(assignmentResolver)
	if !ok {
		return artifact.ProcessResult{}, invalidityError(CategoryContradictoryUsage, scopeID)
	}
	...
	planResults, planDomain := plans.summaries(frames, assignmentResolver, failureResults, scopeID)
	if planDomain != nil {
	...
	if d := writer.writeFactRows(ComponentPlanIndex, toAnySlice(planResults)); d != nil {
		return artifact.ProcessResult{}, d
	}
