...
func compactFrameSummary(summary *FrameSummary) {
	summary.PlanID, summary.TaskID, summary.StepNumber, summary.ParallelGroup, summary.EffectiveConcurrency = nil, nil, nil, nil, nil
...
func frameResultToSummary(f frameResult, ctx TraceContext) FrameSummary {
	return FrameSummary{
		...
		Route:                   f.Route,
		PlanID:                  copyStringPointer(f.PlanID),
		TaskID:                  copyStringPointer(f.TaskID),
		StepNumber:              copyInt64Pointer(f.StepNumber),
		ParallelGroup:           copyStringPointer(f.ParallelGroup),
		EffectiveConcurrency:    copyBoolPointer(f.EffectiveConcurrency),
