...
// PlanUpdate is a complete comparison of two accepted same-plan snapshots.
// A nil Changes denotes unavailable evidence; an empty non-nil list is success.
...
func (update *PlanUpdate) Limited() *PlanUpdate {
	return &PlanUpdate{PreviousSequence: update.PreviousSequence, Availability: "LIMIT_EXCEEDED"}
}
...
type PlanTaskSummary struct {
	...
	AssignedFrameID      *string  `json:"assignedFrameId,omitempty"`
	EffectiveConcurrency *bool    `json:"effectiveConcurrency,omitempty"`
	FailureIDs           []string `json:"failureIds"`
}

type PlanExecutionUnitSummary struct {
	Position             int64    `json:"position"`
	ParallelGroup        *string  `json:"parallelGroup,omitempty"`
	TaskIDs              []string `json:"taskIds"`
	EffectiveConcurrency *bool    `json:"effectiveConcurrency,omitempty"`
	ObservedOverlap      *bool    `json:"observedOverlap,omitempty"`
}

type PlanTransitionSummary struct {
...
type PlanSummary struct {
	...
	ExecutionUnits   []PlanExecutionUnitSummary `json:"executionUnits"`
	Transitions      []PlanTransitionSummary    `json:"transitions"`
}
