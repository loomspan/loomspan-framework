...
// These mirror ai.loomspan.internal.core.TraceRecordType exactly; an
...
// mirror ai.loomspan.internal.core.TraceFrameType exactly.
...
// mirror ai.loomspan.internal.core.TraceOutcome exactly.
...
func normalizeValidationStatus(raw string) (string, bool) {
	for _, canonical := range ValidationStatusValues() {
		if equalFoldASCII(raw, canonical) {
...
func equalFoldASCII(value, canonical string) bool {
	...
	for index := range len(value) {
		...
		if char >= 0x80 || char != canonical[index] {
			return false
		}
	}
	return true
...
// values. These mirror ai.loomspan.internal.runtime.usage.UsagePrecision
