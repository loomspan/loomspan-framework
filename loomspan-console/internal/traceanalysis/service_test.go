...
// consoleCompatibilityVersion "1.0.0-beta.5" emits. Reused from the console
// integration tests so the scope's compatibility gate is established exactly
// as in production.
const javaCompatibleInstanceJSON = `{"instanceId":"11111111-1111-4111-8111-111111111111","consoleCompatibilityVersion":"1.0.0-beta.5","observedAt":"2026-07-25T12:00:00Z","liveMonitoringAvailable":true,"registeredSkillCount":0,"activeExecutionCount":0,"catalogedTraceCount":1,"tracePersistencePolicy":"PERSISTENT","completionGraceTtl":"PT2M","traceCatalogMetadataTtl":"PT168H"}`
...
func newServiceTestHarnessForOutcomeAndVersion(t *testing.T, traceID, ndjson, outcome, compatibilityVersion string) *serviceTestHarness {
	...
	targetContext, err := target.New(func(address applicationclient.Address) (target.ProbeClient, error) {
		return applicationclient.New(address, policy, "1.0.0-beta.5")
	}, nil, time.Now)
	if err != nil {
