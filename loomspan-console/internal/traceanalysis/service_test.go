...
// consoleCompatibilityVersion "1.0.0-beta.5-SNAPSHOT" emits. Reused from the console
// integration tests so the scope's compatibility gate is established exactly
// as in production.
const javaCompatibleInstanceJSON = `{"instanceId":"11111111-1111-4111-8111-111111111111","consoleCompatibilityVersion":"1.0.0-beta.5-SNAPSHOT","observedAt":"2026-07-25T12:00:00Z","liveMonitoringAvailable":true,"registeredSkillCount":0,"activeExecutionCount":0,"catalogedTraceCount":1,"tracePersistencePolicy":"PERSISTENT","completionGraceTtl":"PT2M","traceCatalogMetadataTtl":"PT168H"}`
...
func newServiceTestHarnessForOutcomeAndVersion(t *testing.T, traceID, ndjson, outcome, compatibilityVersion string) *serviceTestHarness {
	...
	targetContext, err := target.New(func(address applicationclient.Address) (target.ProbeClient, error) {
		return applicationclient.New(address, policy, "1.0.0-beta.5-SNAPSHOT")
	}, nil, time.Now)
	if err != nil {
...
func TestFrameDirectRetryCountUsesAttributedLaterAttempts(t *testing.T) {
	...
	raw := strings.Join([]string{
		startedRecord(1),
		frameRecord(2, "initial-frame", "", false, "ROOT_MISSION", true),
		...
		frameRecord(5, "retry-frame", "initial-frame", true, "MODEL_CALL", true),
		withFrame(requestRecord(6, "retry-1", "attempt-2", 2, false), "retry-frame"),
		responseRecord(7, "retry-frame", "retry-1", "attempt-2", 2, 0, 0, 0, "EXACT"),
		frameRecord(8, "retry-frame", "initial-frame", true, "MODEL_CALL", false),
		frameRecord(9, "initial-frame", "", false, "ROOT_MISSION", false),
...
func TestFrameDirectRetryCountCountsLaterAttemptsInSameFrame(t *testing.T) {
	...
	raw := strings.Join([]string{
		startedRecord(1),
		frameRecord(2, "frame", "", false, "ROOT_MISSION", true),
		...
		frameRecord(7, "frame", "", false, "ROOT_MISSION", false),
...
func TestFrameOutcomeIsOptionalScalarAndFiltersSingularly(t *testing.T) {
	closeWithStatus := func(sequence int, status string) string {
		close := frameRecord(sequence, "frame", "", false, "ROOT_MISSION", false)
	...
	for _, test := range []struct {
		...
		{name: "open", frameLines: []string{frameRecord(2, "frame", "", false, "ROOT_MISSION", true)}},
		{name: "blank close", frameLines: []string{frameRecord(2, "frame", "", false, "ROOT_MISSION", true), closeWithStatus(3, "")}},
		{name: "failed close", frameLines: []string{frameRecord(2, "frame", "", false, "ROOT_MISSION", true), closeWithStatus(3, "failed")}, want: &failed},
	} {
...
func TestByteShortenedRecordPagesRecomputeInlineAggregateForAcceptedPage(t *testing.T) {
	...
	for sequence := 2; sequence <= 6; sequence++ {
		lines = append(lines, fmt.Sprintf(`{"traceId":"t","sessionId":"s","sequence":%d,"timestamp":%s,"recordType":"PLAN_VALIDATION_FAILED","frameId":null,"parentFrameId":null,"frameType":null,"route":null,"threadName":"th","metadata":{},"data":%s}`, sequence, timestampForSeq(sequence), content))
	}
	...
	for {
		...
		for _, record := range page.Items {
			if record.Type != string(RecordPlanValidationFailed) {
				continue
			}
			seenRecords++
			if record.Content == nil || len(record.Content.InlineContent) == 0 || record.Content.InlineOmission != "" {
				t.Fatalf("record content was not reconsidered for its accepted page: %+v", record.Content)
			}
		}
		if !page.HasMore {
			break
	...
	if seenRecords != 5 {
		t.Fatalf("seen records=%d, want 5", seenRecords)
	}
}
...
func TestRecordValidationStatusFilterNormalizesRecordedCasing(t *testing.T) {
	record := &Record{Type: RecordStructuredOutputRecorded, Metadata: []byte(`{"status":"RETRYING"}`)}
	if !recordMatchesFilter(record, RecordFilter{ValidationStatus: "retrying"}) {
		t.Fatal("expected lowercase validation filter to match uppercase recorded status")
	}
}

func TestNormalizeValidationStatusUsesClosedCanonicalVocabulary(t *testing.T) {
	for _, test := range []struct {
		raw       string
		canonical string
		ok        bool
	}{
		{raw: "retrying", canonical: "retrying", ok: true},
		{raw: "RETRYING", canonical: "retrying", ok: true},
		{raw: "ReTrYiNg", canonical: "retrying", ok: true},
		{raw: "passed", canonical: "passed", ok: true},
		{raw: "PASSED", canonical: "passed", ok: true},
		{raw: "PaSsEd", canonical: "passed", ok: true},
		{raw: "exhausted", canonical: "exhausted", ok: true},
		{raw: "EXHAUSTED", canonical: "exhausted", ok: true},
		{raw: "ExHaUsTeD", canonical: "exhausted", ok: true},
		{raw: ""},
		{raw: " RETRYING "},
		...
		t.Run(test.raw, func(t *testing.T) {
			canonical, ok := normalizeValidationStatus(test.raw)
			if canonical != test.canonical || ok != test.ok {
				t.Fatalf("normalizeValidationStatus(%q)=(%q,%t), want (%q,%t)", test.raw, canonical, ok, test.canonical, test.ok)
			}
		})
	}
}

func TestRecordValidationStatusFilterKeepsLowercaseRequestContract(t *testing.T) {
	scope := targetEvidence("scope-validation-status")
	for _, status := range ValidationStatusValues() {
		if domain := validateRecordFilter(scope, RecordFilter{ValidationStatus: status}); domain != nil {
			t.Fatalf("lowercase status %q rejected: %v", status, domain)
		}
	}
	for _, status := range []string{"RETRYING", "unknown"} {
		if domain := validateRecordFilter(scope, RecordFilter{ValidationStatus: status}); domain == nil {
			t.Fatalf("unsupported request status %q was accepted", status)
		}
	}
	for _, raw := range []string{"", "unknown", " RETRYING ", "paſſed", "exhauſted"} {
		record := &Record{Metadata: []byte(fmt.Sprintf(`{"status":%q}`, raw))}
		if recordMatchesFilter(record, RecordFilter{ValidationStatus: "retrying"}) {
			t.Fatalf("unsupported recorded status %q matched", raw)
		}
		if summary := recordToSummary(record, recordIndexRow{}, TraceContext{}, RecordRepresentationLogical); summary.ValidationStatus != "" {
			t.Fatalf("unsupported recorded status %q projected as %q", raw, summary.ValidationStatus)
		}
	}
}
