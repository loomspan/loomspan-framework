...
func (w *indexWriter) writeRecordFacts(facts map[int64]persistedRecordFacts) *consolecore.Error {
	...
	for _, sequence := range w.recordSequences {
		...
		if value, ok := facts[sequence]; ok && !value.empty() {
			body, err := marshalRecordFacts(value)
			if err != nil {
				_ = index.Close()
...
func marshalRecordFacts(value persistedRecordFacts) ([]byte, error) {
	body, err := json.Marshal(value)
	if err == nil && len(body) > maxFactRowBytes && value.PlanUpdate != nil {
		value.PlanUpdate = value.PlanUpdate.Limited()
