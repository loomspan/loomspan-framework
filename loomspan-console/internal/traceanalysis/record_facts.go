...
func (facts persistedRecordFacts) empty() bool {
	return facts.Plan == nil && facts.PlanUpdate == nil && len(facts.Attempts) == 0 && len(facts.Retries) == 0 && len(facts.Validations) == 0 && len(facts.Failures) == 0 && len(facts.Payloads) == 0
}
...
func buildPersistedRecordFacts(plans map[int64]PlanReference, updates map[int64]*PlanUpdate, attempts []attemptResult, retries []retryResult, validations []validationLink, failures []failureResult, payloads []payloadIndexRow) map[int64]persistedRecordFacts {
	...
	for sequence, plan := range plans {
		...
		facts := out[sequence]
		facts.Plan = &copy
		facts.PlanUpdate = updates[sequence]
		out[sequence] = facts
	}
...
func materializeRecordFacts(stored persistedRecordFacts, rec *Record, filter RecordFilter, ref evidence.Reference, handle artifact.Handle, traceCtx TraceContext) (RecordFacts, error) {
	facts := RecordFacts{PlanUpdate: stored.PlanUpdate, Attempts: []AttemptSummary{}, Retries: []RetrySummary{}, Validations: []ValidationSummary{}, Failures: []FailureSummary{}, Payloads: []PayloadDescriptor{}, SearchMatches: []SearchResult{}}
	if stored.Plan != nil {
		copy := *stored.Plan
		facts.Plan = &copy
	}
