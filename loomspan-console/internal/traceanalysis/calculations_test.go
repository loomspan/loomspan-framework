...
func startedRecord(seq int) string {
	return `{"traceId":"t","sessionId":"s","sequence":` + itoa(seq) +
		`,"timestamp":` + timestampForSeq(seq) +
		`,"recordType":"TRACE_STARTED","frameId":null,"parentFrameId":null,"frameType":null,"route":null,"threadName":"th","metadata":{"consoleCompatibilityVersion":"development","generationId":"generation-test"},"data":null}`
}
...
func TestFramesRejectDuplicateMissingSelfParentCycleAndConflictingIdentity(t *testing.T) {
	...
	t.Run("multiple_roots", func(t *testing.T) {
		raw := startedRecord(1) + "\n" +
			frameRecord(2, "root-a", "", false, "ROOT_MISSION", true) + "\n" +
			frameRecord(3, "root-b", "", false, "ROOT_MISSION", true) + "\n" +
			completionRecord(4, "SUCCEEDED", 0, 0, 0, "") + "\n"
		_, cat, ok := processTrace(t, raw)
		if ok || cat != CategoryInvalidFrameRelationship {
			t.Fatalf("expected INVALID_FRAME_RELATIONSHIP for multiple roots, got ok=%v category=%s", ok, cat)
		}
	})

	t.Run("non_root_first_frame", func(t *testing.T) {
		raw := startedRecord(1) + "\n" +
			frameRecord(2, "model", "", false, "MODEL_CALL", true) + "\n" +
			completionRecord(3, "SUCCEEDED", 0, 0, 0, "") + "\n"
		_, cat, ok := processTrace(t, raw)
		if ok || cat != CategoryInvalidFrameRelationship {
			t.Fatalf("expected INVALID_FRAME_RELATIONSHIP for non-root first frame, got ok=%v category=%s", ok, cat)
		}
	})
...
// TestFramesUnionOverlappingChildrenAndMarkIncompleteUnavailable proves that
// overlap is counted once while an incomplete immediate child remains uncertain.
func TestFramesUnionOverlappingChildrenAndMarkIncompleteUnavailable(t *testing.T) {
	t.Run("overlapping_children", func(t *testing.T) {
		...
		// Child union is [100s,400s], so self duration is 100000ms.
		if root.SelfDurationMillis == nil || *root.SelfDurationMillis != 100000 {
			t.Errorf("root self duration=%v want 100000", root.SelfDurationMillis)
		}
		...
		if m.UncertaintyCount != 0 {
			t.Fatalf("expected no uncertainty, got %d", m.UncertaintyCount)
		}
	})
...
func TestFramesUnionSelfDurationMatrix(t *testing.T) {
	cases := []struct {
		name      string
		intervals [][2]int64
		want      int64
	}{
		{"disjoint", [][2]int64{{10, 20}, {30, 40}}, 80},
		{"adjacent", [][2]int64{{10, 20}, {20, 30}}, 80},
		{"partial-overlap", [][2]int64{{10, 30}, {20, 40}}, 70},
		{"nested", [][2]int64{{10, 50}, {20, 30}}, 60},
		{"identical", [][2]int64{{10, 30}, {10, 30}}, 80},
		{"zero-width", [][2]int64{{10, 10}, {20, 30}}, 90},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			graph := newFrameGraph()
			parent := &frameBuild{frameID: "root", openedMillis: 0, closedMillis: 100, closed: true}
			graph.frames[parent.frameID] = parent
			for index, interval := range tc.intervals {
				id := fmt.Sprintf("child-%d", index)
				parent.children = append(parent.children, id)
				graph.frames[id] = &frameBuild{frameID: id, openedMillis: interval[0], closedMillis: interval[1], closed: true}
			}
			var uncertainties []uncertaintyResult
			got := computeSelfDuration(parent, graph, &uncertainties)
			if got == nil || *got != tc.want || len(uncertainties) != 0 {
				t.Fatalf("got=%v uncertainty=%+v want=%d", got, uncertainties, tc.want)
			}
		})
	}
}

func TestFramesRejectDurationOverflowAsInvalidRelationship(t *testing.T) {
	graph := &frameGraph{frames: map[string]*frameBuild{"root": {frameID: "root", openedMillis: math.MinInt64, closedMillis: math.MaxInt64, opened: true, closed: true}}, order: []string{"root"}, rootID: "root"}
	if domain := graph.validate(); domain == nil {
		t.Fatal("overflowing frame interval accepted")
	} else if category, ok := categoryOf(domain); !ok || category != CategoryInvalidFrameRelationship {
		t.Fatalf("domain=%v category=%v", domain, category)
	}
}
