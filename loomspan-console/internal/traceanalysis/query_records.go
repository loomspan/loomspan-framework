...
func recordMatchesFilter(rec *Record, f RecordFilter) bool {
	...
	if f.ValidationStatus != "" {
		rawStatus, _, _ := rec.metadataString("status")
		status, ok := normalizeValidationStatus(rawStatus)
		if !ok || status != f.ValidationStatus {
			return false
		}
	}
	if f.FailureID != "" {
...
func recordToSummary(rec *Record, row recordIndexRow, ctx TraceContext, rep RecordRepresentation) RecordSummary {
	...
	validationStatus, _ := normalizeValidationStatus(rec.metadataStringOrEmpty("status"))
