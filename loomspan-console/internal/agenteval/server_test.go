...
func TestEvaluationServerUsesProductionAdapterAndThirteenTools(t *testing.T) {
	...
	server, err := StartServer(t.TempDir(), cases["finalized-tools-only"], "1.0.0-beta.5-SNAPSHOT")
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestLiveEvaluationCaseServesCompleteBranchesThroughProductionAdapter(t *testing.T) {
	...
	server, err := StartServer(t.TempDir(), cases["live-tools-only"], "1.0.0-beta.5-SNAPSHOT")
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestImportedEvaluationCasePresentsAmbiguousSameTimeCandidates(t *testing.T) {
	...
	server, err := StartServer(t.TempDir(), cases["imported-tools-only"], "1.0.0-beta.5-SNAPSHOT")
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
