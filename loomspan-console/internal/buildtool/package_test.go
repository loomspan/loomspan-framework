	...
	for _, coordinates := range [][2]string{{"windows", "amd64"}, {"linux", "amd64"}, {"darwin", "arm64"}} {
		t.Run(coordinates[0]+"-"+coordinates[1], func(t *testing.T) {
			...
			for _, relative := range agentskills.RuntimeDebuggingFiles {
				...
				for _, retired := range []string{"loomspan.runtime-status.v1", "loomspan.trace-inspection.v1", "all twelve", "MCP plan presentation remains deferred"} {
					if bytes.Contains(contents[name], []byte(retired)) {
						t.Errorf("archive skill %s contains retired guidance %q", relative, retired)
					}
				}
			}
		})
