...
func TestEveryInstalledToolRepresentativeOutputsValidateAgainstBothSchemas(t *testing.T) {
	...
	tests := []struct {
		...
		{"skill-detail", func(t *testing.T) {
			validateTypedEnvelope(t, "skill-detail", skillDetailOutputSchema(), skillDetailResult{ObservedAt: now, Skill: skillDetailDTO{RegisteredName: "skill", Source: "YAML", SourcePath: "skill.yaml", YAML: "name: skill"}})
		}},
		{"executions-list", func(t *testing.T) {
			validateTypedEnvelope(t, "executions-list", executionListOutputSchema(), executionListResult{ObservedAt: now, Items: []executionListItemDTO{}})
		}},
		{"execution-detail", func(t *testing.T) {
			validateTypedEnvelope(t, "execution-detail", executionDetailOutputSchema(), executionDetailResult{ObservedAt: now, Execution: executionDetailDTO{SessionID: "session", TraceID: "trace", Status: "RUNNING", Phase: "STEP", ActiveBranches: []activeBranchDTO{}}})
		}},
		...
		{"plans", func(t *testing.T) {
			validateTypedEnvelope(t, "plans", planQueryOutputSchema(), queryPlansResult{Evidence: evidence, Items: []planDTO{}})
		}},
		{"records", func(t *testing.T) {
			validateTypedEnvelope(t, "records", recordQueryOutputSchema(), queryRecordsResult{Evidence: evidence, Items: []recordDTO{{Sequence: 1, Type: "MODEL_RESPONSE_RECEIVED", ValidationStatus: "retrying", TimestampMillis: 1, Representation: "LOGICAL", Attempts: []attemptDTO{}, Retries: []retryDTO{}, Validations: []validationDTO{}, Failures: []failureDTO{}, Content: &contentDescriptorDTO{Role: "DATA", Available: true, Complete: true, ContentRef: "opaque"}}}})
...
func TestCompactSchemasRetainDecisionAndNavigationFields(t *testing.T) {
	...
	recordValidationStatus := recordQueryOutputSchema().Properties["result"].Properties["items"].Items.Properties["validationStatus"]
	if recordValidationStatus == nil || recordValidationStatus.Type != "string" || slices.Contains(recordQueryOutputSchema().Properties["result"].Properties["items"].Items.Required, "validationStatus") {
		t.Fatalf("optional record validationStatus schema=%+v", recordValidationStatus)
	}
	...
	tests := []struct {
		...
		{"runtime", runtimeOutputSchema(), map[string]any{"observedAt": "2026-08-19T00:00:00Z", "targetSelection": "NONE", "targetConnection": "DISCONNECTED", "targetAuthentication": "UNKNOWN", "javaGoCompatibility": "UNKNOWN", "runtimeIdentity": "UNKNOWN", "liveMonitoring": "UNAVAILABLE"}},
		{"skills-list", skillListOutputSchema(), map[string]any{"result": map[string]any{"observedAt": "2026-08-19T00:00:00Z", "items": []any{map[string]any{"registeredName": "skill", "source": "YAML", "sourcePath": "skill.yaml"}}, "hasMore": false}}},
		{"skill-detail", skillDetailOutputSchema(), map[string]any{"result": map[string]any{"observedAt": "2026-08-19T00:00:00Z", "skill": map[string]any{"registeredName": "skill", "source": "YAML", "sourcePath": "skill.yaml", "yaml": "name: skill"}}}},
		{"executions-list", executionListOutputSchema(), map[string]any{"result": map[string]any{"observedAt": "2026-08-19T00:00:00Z", "items": []any{map[string]any{"sessionId": "s", "traceId": "t", "status": "RUNNING", "phase": "STEP", "updatedAt": "2026-08-19T00:00:00Z", "activeBranchCount": 0}}, "hasMore": false}}},
		{"execution-detail", executionDetailOutputSchema(), map[string]any{"result": map[string]any{"observedAt": "2026-08-19T00:00:00Z", "execution": map[string]any{"sessionId": "s", "traceId": "t", "status": "RUNNING", "phase": "STEP", "activeBranches": []any{}, "usage": map[string]any{}, "configuredLimits": map[string]any{}}}}},
		{"activity", activityOutputSchema(), map[string]any{"result": map[string]any{"observedAt": "2026-08-19T00:00:00Z", "items": []any{map[string]any{"cursor": "c", "sessionId": "s", "traceId": "t", "timestamp": "2026-08-19T00:00:00Z", "kind": "STEP_COMPLETED", "facts": map[string]any{}}}, "hasMore": false, "coverage": map[string]any{}}}},
		...
		{"trace-summary", traceSummaryOutputSchema(), map[string]any{"result": map[string]any{"evidence": compactEvidenceInstance(), "summary": map[string]any{"outcome": "FAILED", "terminalFailureId": "failure-terminal", "recordCount": 1, "recordCountsByType": map[string]any{"TRACE_COMPLETED": 1}, "frameCount": 1, "planCount": 1, "attemptCount": 1, "retryCount": 0, "failureCount": 1, "rootFrameIds": []any{"root"}, "usageComplete": true}}}},
		{"plans", planQueryOutputSchema(), map[string]any{"result": map[string]any{"evidence": compactEvidenceInstance(), "items": []any{}, "hasMore": false}}},
		...
		{"records", recordQueryOutputSchema(), map[string]any{"result": map[string]any{"evidence": compactEvidenceInstance(), "items": []any{map[string]any{"sequence": 1, "type": "STEP_COMPLETED", "validationStatus": "retrying", "timestampMillis": 1, "representation": "LOGICAL", "attempts": []any{}, "retries": []any{}, "validations": []any{}, "failures": []any{}}}, "hasMore": false}}},
		{"record-content", recordQueryOutputSchema(), map[string]any{"result": map[string]any{"evidence": compactEvidenceInstance(), "items": []any{map[string]any{"sequence": 1, "type": "MODEL_RESPONSE_RECEIVED", "timestampMillis": 1, "representation": "LOGICAL", "attempts": []any{}, "retries": []any{}, "validations": []any{}, "failures": []any{}, "content": map[string]any{"role": "DATA", "available": true, "complete": true, "contentRef": "opaque"}}}, "hasMore": false}}},
...
func TestActiveCompactSchemasExposeFactsAndNoDerivedDiagnosticStates(t *testing.T) {
	execution := compactExecutionDetailSchema()
	for _, name := range []string{"sessionId", "traceId", "lastCanonicalSequence", "startedAt", "updatedAt", "elapsedMillis", "entrySkill", "status", "phase", "activeBranches", "usage", "configuredLimits"} {
		if execution.Properties[name] == nil {
			t.Errorf("execution schema does not advertise %q", name)
		}
	}
	for _, removed := range []string{"summary", "activePath", "totalFrameDepth", "activePathTruncated"} {
		if execution.Properties[removed] != nil {
			t.Errorf("obsolete execution field remains: %s", removed)
		}
	}
...
func TestTraceSummaryHistogramSchemaRejectsUnknownAndInvalidCounts(t *testing.T) {
	base := map[string]any{"result": map[string]any{"evidence": compactEvidenceInstance(), "summary": map[string]any{"outcome": "SUCCEEDED", "recordCount": 1, "recordCountsByType": map[string]any{"TRACE_STARTED": 1}, "frameCount": 0, "planCount": 0, "attemptCount": 0, "retryCount": 0, "failureCount": 0, "rootFrameIds": []any{}, "usageComplete": true}}}
	...
	for name, counts := range map[string]map[string]any{
		...
		if err := resolved.Validate(instance); err != nil {
			t.Fatalf("compact discovery rejected deep histogram %s: %v", name, err)
		}
