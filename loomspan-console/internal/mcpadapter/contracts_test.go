...
func TestPR17TextFallbacksHaveExactOrderEscapingAndFinalNewline(t *testing.T) {
	result := skillListResult{
		...
		Items:      []skillSummaryDTO{{RegisteredName: "name\nquoted\"", Source: "YAML", SourcePath: "skills/a.yaml"}},
	}
...
func TestRejectedPropertySpellingsInsideUntrustedEvidenceRoundTripUnchanged(t *testing.T) {
	...
	if encoded, _ := json.Marshal(activity); strings.Contains(string(encoded), "artifactHandle") || strings.Contains(string(encoded), "targetScopeId") {
		t.Fatalf("unselected activity properties leaked: %s", encoded)
	}
	...
	if record.Content == nil || record.Content.InlineContent != sentinel || strings.Contains(string(encodedRecord), "contextSummary") {
...
func walkMCPProperties(t *testing.T, typ reflect.Type, path string, forbidden map[string]bool, seen map[reflect.Type]bool) {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		fieldPath := path + "." + name
		if forbidden[name] && !(name == "source" && (typ == reflect.TypeOf(skillSummaryDTO{}) || typ == reflect.TypeOf(skillDetailDTO{}))) {
