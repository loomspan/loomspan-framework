...
func TestActiveInspectionDualOutputBaselines(t *testing.T) {
	...
	maximumExecutions.Items = make([]executionListItemDTO, maxMCPPageSize)
	...
	for index := range maximumActivity.Items {
		...
		maximumActivity.Items[index] = activityDTO{
			...
			FrameType:         "MODEL",
			Route:             "model",
			Facts:             activityFactsDTO{AttemptNumber: func() *json.Number { value := json.Number("9007199254740993"); return &value }()},
		}
	}
	activityAdmission := newActivityPageAdmission()
	admittedActivityCount := 0
	for index, item := range maximumActivity.Items {
		if !activityAdmission.admit(item, activityItemText(item, index)) {
			break
		}
		admittedActivityCount++
	}
	maximumActivity.Items = maximumActivity.Items[:admittedActivityCount]
	maximumActivity.ReturnedCursorRange.LastCursor = maximumActivity.Items[admittedActivityCount-1].Cursor
	maximumActivity.Continuation = "continuation-after-budgeted-activities"
	...
	tests := []struct {
	...
	}{
		{
			...
			dualOutputMeasurement{FullResponse: 589, TextWireContribution: 307, StructuredWireContribution: 234},
		},
		{
		...
		},
		{
			...
			dualOutputMeasurement{FullResponse: 2255, TextWireContribution: 1249, StructuredWireContribution: 958},
		},
		{
			...
			dualOutputMeasurement{FullResponse: 22673, TextWireContribution: 13457, StructuredWireContribution: 9168},
		},
		{
			"activity, maximum page", toolEnvelope[activityResult]{Result: &maximumActivity}, activityText(maximumActivity),
			dualOutputMeasurement{FullResponse: 29372, TextWireContribution: 17354, StructuredWireContribution: 11970},
		},
	}
