...
func addExecutionTools(server *mcp.Server, options ServerOptions) {
	addValidatedTool(server, &mcp.Tool{
		...
		Description: "List compact active execution facts.",
	...
	addValidatedTool(server, &mcp.Tool{
		...
		Description: "Return every active branch for one session ID.",
...
func handleListExecutions(ctx context.Context, options ServerOptions, input listExecutionsInput) (*mcp.CallToolResult, toolEnvelope[executionListResult], error) {
	...
	items := make([]executionListItemDTO, 0, len(page.Items))
	for _, item := range page.Items {
		items = append(items, mapExecutionListItem(item))
	}
	continuation := ""
...
func handleGetExecution(ctx context.Context, options ServerOptions, input getExecutionInput) (*mcp.CallToolResult, toolEnvelope[executionDetailResult], error) {
	...
	result := executionDetailResult{
		ObservedAt: options.Now().UTC(), Execution: mapExecutionDetail(execution),
	}
	fallback := executionDetailText(result)
	if !fitsOrdinaryResponse(result, fallback) {
		domain := consolecore.NewError(consolecore.CodeLimitExceeded,
			"The active execution detail exceeds the safe response budget.", string(scope.ID),
	...
	return successResult(result, fallback)
}
...
func executionListText(result executionListResult) string {
	...
	for index, item := range result.Items {
		...
		writer.time(prefix+"updatedAt", item.UpdatedAt)
		writer.integer(prefix+"activeBranchCount", int64(item.ActiveBranchCount))
	}
...
func appendExecutionText(writer *lineWriter, prefix string, execution executionDetailDTO) {
	...
	for branchIndex, branch := range execution.ActiveBranches {
		branchPrefix := fmt.Sprintf("%sactiveBranches[%d].", prefix, branchIndex)
		appendNullableString(writer, branchPrefix+"planId", branch.PlanID)
		appendNullableString(writer, branchPrefix+"taskId", branch.TaskID)
		if branch.StepNumber == nil {
			writer.lines = append(writer.lines, branchPrefix+"stepNumber: null")
		} else {
			writer.integer(branchPrefix+"stepNumber", int64(*branch.StepNumber))
		}
		appendNullableString(writer, branchPrefix+"parallelGroup", branch.ParallelGroup)
		if branch.EffectiveConcurrency == nil {
			writer.lines = append(writer.lines, branchPrefix+"effectiveConcurrency: null")
		} else {
			writer.boolean(branchPrefix+"effectiveConcurrency", *branch.EffectiveConcurrency)
		}
		writer.integer(branchPrefix+"path.count", int64(len(branch.Path)))
		for pathIndex, entry := range branch.Path {
			pathPrefix := fmt.Sprintf("%spath[%d].", branchPrefix, pathIndex)
...
func appendNullableString(writer *lineWriter, name string, value *string) {
	if value == nil {
		writer.lines = append(writer.lines, name+": null")
		return
	}
	writer.quoted(name, *value)
}
