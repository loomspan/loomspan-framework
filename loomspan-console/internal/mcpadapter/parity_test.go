...
func TestBrowserAndMCPPreserveSameActiveExecutionFacts(t *testing.T) {
	...
	want := mapExecutionDetail(browserFact)
...
func TestBrowserAndMCPPreserveSameRecentContinuityAndGapFacts(t *testing.T) {
	...
	browserItem := live.Activity{
		...
		Kind: live.KindModelAttemptFailed, Summary: "failed", Details: json.RawMessage(`{"attemptNumber":2}`),
	}
	...
	var roundTrip activityDTO
	if err := json.Unmarshal(encoded, &roundTrip); err != nil {
		t.Fatal(err)
	}
	if roundTrip.Cursor != browserItem.Cursor || roundTrip.SessionID != browserItem.SessionID ||
		roundTrip.TraceID != browserItem.TraceID || !reflect.DeepEqual(roundTrip.CanonicalSequence, browserItem.CanonicalSequence) ||
		!roundTrip.Timestamp.Equal(browserItem.Timestamp) || roundTrip.Kind != browserItem.Kind || roundTrip.Facts.AttemptNumber == nil || string(*roundTrip.Facts.AttemptNumber) != "2" {
...
func TestBrowserAndMCPPreserveJavaSkillFixture(t *testing.T) {
	body, err := os.ReadFile(filepath.Join("..", "..", "..", "loomspan-console-fixtures", "application-rest", "skill-java-detail.json"))
	if err != nil {
		t.Fatal(err)
	}
	var browser observability.SkillDetail
	if err := json.Unmarshal(body, &browser); err != nil {
		t.Fatal(err)
	}
	options := newMCPTestOptions(t, func(string) ([]byte, error) { return body, nil })
	text, envelope, err := handleGetSkill(context.Background(), options, getSkillInput{RegisteredName: browser.RegisteredName})
	if err != nil || envelope.Result == nil {
		t.Fatalf("envelope=%#v err=%v", envelope, err)
	}
	got := envelope.Result.Skill
	if got.Source != browser.Source || got.RegisteredName != browser.RegisteredName || got.BeanName != browser.BeanName || got.Method != browser.Method || got.YAML != "" || got.SourcePath != "" {
		t.Fatalf("browser=%#v MCP=%#v", browser, got)
	}
	validateTypedEnvelope(t, "skill-detail", skillDetailOutputSchema(), *envelope.Result)
	if text == nil {
		t.Fatal("missing Java diagnostic text")
	}
}
