...
// TestPR18SemanticFixtures keeps the transport-neutral semantic scenarios
// executable after removal of the obsolete capability manifest.
func TestPR18SemanticFixtures(t *testing.T) {
	...
	for _, id := range []string{"trace.target-acquisition", "trace.target-free-import", "trace.trace-id-resolution", "trace.ambiguous-identity", "trace.discovery-completeness", "trace.parity", "trace.fact-projection", "trace.continuation", "trace.lifecycle", "trace.cancellation", "trace.unavailable-evidence", "trace.concurrent-clients", "trace.joined-adapters", "trace.schema-errors", "raw.exact-range", "raw.trace-id-continuation", "raw.lifecycle-errors", "raw.resolver-only", "raw.inert-content"} {
		runner := runners[id]
		if runner == nil {
...
func fixtureFactProjection(t *testing.T) {
	...
	for _, record := range page.Items {
		...
		if len(mapped.Attempts) != 1 || mapped.Attempts[0].AttemptID != "attempt-1" || len(mapped.Retries) != 1 || mapped.Retries[0].RetrySequenceID != "retry-1" || mapped.Failures == nil {
			t.Fatalf("persisted facts=%#v", mapped)
		}
		return
	}
...
func fixtureTraceParity(t *testing.T) {
	...
	raw = bytes.ReplaceAll(raw, []byte(`"consoleCompatibilityVersion":"1.0.0-beta.6-SNAPSHOT"`), []byte(`"consoleCompatibilityVersion":"development"`))
	...
	_, planEnvelope, err := handleQueryTracePlans(context.Background(), options, queryTracePlansInput{TraceID: h.acquired.Metadata.TraceID, PageSize: 64})
	if err != nil || planEnvelope.Result == nil {
		t.Fatalf("MCP plans=%#v err=%v", planEnvelope, err)
	}
	assertAdapterItemsEqual(t, browserPost("/api/console/v1/traces/analysis/plans", body)["items"], planEnvelope.Result.Items)
	...
	assertAdapterItemsEqual(t, browserPost("/api/console/v1/traces/analysis/records", body)["items"], recordEnvelope.Result.Items, "raw", "facts", "threadName", "isChunk", "isEnvelope", "attempts", "retries", "validations", "failures")
...
func TestPlanAndSearchAdaptersPreserveSameSemantics(t *testing.T) {
	...
	for index, item := range browserPlans {
		...
		encoded, marshalErr := json.Marshal(allRecords[index].Plan)
		if marshalErr != nil {
			t.Fatal(marshalErr)
		}
		var mcpPlan any
...
func TestValidationOutcomeStatusIsConsistentAcrossMCPAndBrowser(t *testing.T) {
	raw, err := os.ReadFile(filepath.Join("..", "..", "..", "loomspan-console-fixtures", "traces", "advisor-retry.ndjson"))
	if err != nil {
		t.Fatal(err)
	}
	raw = bytes.ReplaceAll(raw, []byte(`"consoleCompatibilityVersion":"1.0.0-beta.6-SNAPSHOT"`), []byte(`"consoleCompatibilityVersion":"development"`))
	...
	for _, test := range []struct {
		...
		{status: "retrying", sequence: 5},
		{status: "passed", sequence: 10},
	} {
		t.Run(test.status, func(t *testing.T) {
			call, envelope, err := handleQueryTraceRecords(context.Background(), options, queryTraceRecordsInput{
				TraceID: traceID, PageSize: 1,
				Filter: traceanalysis.RecordFilter{Types: []string{string(traceanalysis.RecordStructuredOutputRecorded)}, ValidationStatus: test.status},
			})
			if err != nil || call.IsError || envelope.Result == nil || len(envelope.Result.Items) != 1 || envelope.Result.HasMore {
				t.Fatalf("MCP status query=%#v err=%v", envelope, err)
			}
			mcpRecord := envelope.Result.Items[0]
			if mcpRecord.Sequence != test.sequence || mcpRecord.ValidationStatus != test.status || len(mcpRecord.Validations) != 0 {
				t.Fatalf("MCP record=%#v", mcpRecord)
			}
			fallback := call.Content[0].(*mcp.TextContent).Text
			if !strings.Contains(fallback, fmt.Sprintf(`"validationStatus":%q`, test.status)) {
				t.Fatalf("fallback=%q", fallback)
			}

			browser := browserPost("/api/console/v1/traces/analysis/records", fmt.Sprintf(
				`{"source":"IMPORTED","traceId":%q,"pageSize":1,"filter":{"types":["STRUCTURED_OUTPUT_RECORDED"],"validationStatus":%q}}`, traceID, test.status))
			items := browser["items"].([]any)
			if len(items) != 1 {
				t.Fatalf("browser items=%#v", items)
			}
			browserRecord := items[0].(map[string]any)
			if browserRecord["sequence"] != float64(test.sequence) || browserRecord["validationStatus"] != test.status {
				t.Fatalf("browser record=%#v", browserRecord)
			}
			if _, exists := browserRecord["facts"]; exists {
				t.Fatalf("browser record exposed relationship facts: %#v", browserRecord)
			}
		})
	}
}
...
func TestFrameRetryMinimumAndAttemptFailureFallbackUseRealParsedFacts(t *testing.T) {
	t.Run("frame filter", func(t *testing.T) {
		...
		if err != nil || envelope.Result == nil || len(envelope.Result.Items) != 1 || envelope.Result.Items[0].FrameID != "planning-primary" || envelope.Result.Items[0].DirectRetryCount != 0 {
	...
	t.Run("attempt failure fallback", func(t *testing.T) {
		...
		attempts := envelope.Result.Items[0].Attempts
...
func TestRecoveredProviderAttemptDiagnosticUsesExistingRecordAndContentTools(t *testing.T) {
	raw, err := os.ReadFile(filepath.Join("..", "..", "..", "loomspan-console-fixtures", "traces", "recovered-provider-attempt-diagnostic.ndjson"))
	if err != nil {
		t.Fatal(err)
	}
	raw = bytes.ReplaceAll(raw, []byte(`"consoleCompatibilityVersion":"1.0.0-beta.6-SNAPSHOT"`), []byte(`"consoleCompatibilityVersion":"development"`))
	...
	call, envelope, err := handleQueryTraceRecords(context.Background(), options, queryTraceRecordsInput{
		TraceID: h.acquired.Metadata.TraceID, Representation: traceanalysis.RecordRepresentationLogical, PageSize: 10,
		Filter: traceanalysis.RecordFilter{
			Types: []string{string(traceanalysis.RecordModelAttemptFailed)}, FrameID: "model",
	...
	if err != nil || call.IsError || envelope.Result == nil || len(envelope.Result.Items) != 1 {
		t.Fatalf("failed-attempt query=%#v err=%v", envelope, err)
	}
	record := envelope.Result.Items[0]
	if record.Content == nil || record.Content.ContentRef == "" || record.Content.RetainedBytes <= 8192 || record.Content.InlineContent != "" ||
		len(record.Attempts) != 1 || record.Attempts[0].AttemptID != "attempt-provider-1" ||
		record.Attempts[0].ContentRef != record.Content.ContentRef || record.FailureID != "" || len(record.Failures) != 0 {
		t.Fatalf("record=%#v", record)
	}
	fallback := call.Content[0].(*mcp.TextContent).Text
	for _, want := range []string{`"attemptId":"attempt-provider-1"`, `"retrySequenceId":"retry-provider"`, `"failureClassification":"TRANSIENT"`, `"failureCategory":"TIMEOUT"`, `"retryDecision":"RETRY"`, `"contentRef":"`} {
		if !strings.Contains(fallback, want) {
			t.Fatalf("fallback missing %q: %q", want, fallback)
		}
	}
	for _, omitted := range []string{"fixture provider read timeout", "fixture suppressed context", "gateway timeout evidence", "cause=", "exceptionMessage=", "recovered=", "terminal="} {
		if strings.Contains(fallback, omitted) {
			t.Fatalf("descriptor-default fallback exposed or invented %q: %q", omitted, fallback)
		}
	}

	var content strings.Builder
	input := traceRangeInput{TraceID: h.acquired.Metadata.TraceID, ContentRef: record.Content.ContentRef, MaxBytes: 97}
	for {
		result, rangeEnvelope, rangeErr := handleTraceRange(context.Background(), options, input, false)
		if rangeErr != nil || result.IsError || rangeEnvelope.Result == nil {
			t.Fatalf("content range=%#v err=%v", rangeEnvelope, rangeErr)
		}
		page := rangeEnvelope.Result
		if page.ContentType != "application/json" || page.Encoding != string(traceanalysis.RangeEncodingText) || page.ActualStart != int64(content.Len()) {
			t.Fatalf("range=%#v", page)
		}
		...
		input = traceRangeInput{TraceID: h.acquired.Metadata.TraceID, ContentRef: record.Content.ContentRef, Continuation: page.Continuation, MaxBytes: 97}
	}
	if !json.Valid([]byte(content.String())) {
		t.Fatalf("content was not complete JSON")
	}
	for _, want := range []string{"JAVA_STACK_TRACE", "fixture provider read timeout", "fixture suppressed context", "PROVIDER_ERROR", "gateway timeout evidence"} {
		if !strings.Contains(content.String(), want) {
			t.Fatalf("complete content missing %q", want)
		}
	}

	browserPost := newSemanticBrowserPost(t, h)
	browserRange := browserPost("/api/console/v1/traces/analysis/content-range", fmt.Sprintf(
		`{"source":"IMPORTED","traceId":%q,"contentRef":%q,"maxBytes":16384}`,
		h.acquired.Metadata.TraceID, record.Content.ContentRef))
	if browserRange["content"] != content.String() || browserRange["contentType"] != "application/json" || browserRange["hasMore"] != false {
		t.Fatalf("browser range did not preserve recovered diagnostic content: %#v", browserRange)
	}
}
...
func assertIndependentSemanticFacts(t *testing.T, browserPost func(string, string) map[string]any, body string, options ServerOptions, handle artifact.Handle, frames []frameDTO, records []recordDTO) map[string]int {
	...
	for _, record := range records {
		encoded, err := json.Marshal(map[string]any{"attempts": record.Attempts, "retries": record.Retries, "validations": record.Validations, "failures": record.Failures})
		if err != nil {
			t.Fatal(err)
		}
