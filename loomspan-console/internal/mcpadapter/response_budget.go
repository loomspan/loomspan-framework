...
func newInlineRecordPageAdmission(traceID string) *pageAdmission {
	...
	fallbackTraceID, _ := json.Marshal(traceID)
...
// Activity pages have a small fixed envelope but can contain up to 64
// independently bounded events in both structured and text form.
func newActivityPageAdmission() *pageAdmission {
	return &pageAdmission{budget: defaultTraceResultBudget, used: 4 << 10}
}

func fitsOrdinaryResponse(structured any, fallback string) bool {
	return newPageAdmission().admit(structured, fallback)
}
