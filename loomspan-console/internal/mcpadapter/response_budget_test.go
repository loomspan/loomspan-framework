...
func TestAttemptFailureFallbackParticipatesInWholeItemAdmission(t *testing.T) {
	item := recordDTO{Sequence: 1, Type: string(traceanalysis.RecordModelAttemptFailed), ValidationStatus: "retrying", Attempts: []attemptDTO{{
	...
	if !strings.Contains(fallback, `"validationStatus":"retrying"`) {
		t.Fatalf("fallback omitted validation status: %q", fallback)
	}
...
func TestFullSerializedDefaultTraceResultsMeetCommittedBudgets(t *testing.T) {
	...
	evidence := evidenceDTO{TraceID: "trace-budget", SessionID: "session", ObservedAt: time.Date(2026, 8, 19, 0, 0, 0, 0, time.UTC)}
	measurements := map[string]int{}

	inventory := listTracesResult{ObservedAt: evidence.ObservedAt, Complete: true, HasMore: true, Continuation: "opaque"}
	...
	frames := queryFramesResult{Evidence: evidence, Projection: string(traceanalysis.FrameProjectionDetailed), HasMore: true, Continuation: "opaque"}
	...
	records := queryRecordsResult{Evidence: evidence, HasMore: true, Continuation: "opaque"}
	recordAdmission := newPageAdmission()
	for index := range 64 {
		item := recordDTO{Sequence: int64(index + 1), Type: "MODEL_RESPONSE_RECEIVED", FrameID: strings.Repeat("f", 2048), Representation: "logical", Attempts: []attemptDTO{}, Retries: []retryDTO{}, Validations: []validationDTO{}, Failures: []failureDTO{}, Content: &contentDescriptorDTO{Role: "DATA", ContentType: "application/json", Encoding: "UTF8", Available: true, Complete: true, ContentRef: strings.Repeat("r", 4096)}}
	...
	for index := range 4 {
		item := recordDTO{Sequence: int64(index + 1), Type: "MODEL_RESPONSE_RECEIVED", Representation: "logical", Attempts: []attemptDTO{}, Retries: []retryDTO{}, Validations: []validationDTO{}, Failures: []failureDTO{}, Content: &contentDescriptorDTO{Role: "DATA", ContentType: "application/json", Encoding: "UTF8", RetainedBytes: traceanalysis.MaxInlineContentBytes, Available: true, Complete: true, InlineEligibility: true, ContentRef: "opaque", InlineContent: strings.Repeat("v", traceanalysis.MaxInlineContentBytes)}}
	...
	textRange := rangeResult{Evidence: evidence, ActualEnd: int64(len(worstText)), TotalLength: int64(len(worstText) + 1), ContentType: "text/plain", Encoding: "TEXT", Content: worstText, HasMore: true, Continuation: "opaque"}
	...
	binaryRange := rangeResult{Evidence: evidence, ActualEnd: int64(traceanalysis.DefaultRangeBytes), TotalLength: int64(traceanalysis.DefaultRangeBytes + 1), ContentType: "application/octet-stream", Encoding: "BASE64", Content: binary, HasMore: true, Continuation: "opaque"}
	...
	want := map[string]int{
		...
		"detailed frames":     445,
		"compact frames":      443,
		"record descriptors":  389,
		"inline records":      17521,
		"semantic text range": 13915,
		"raw base64 range":    3371,
	}
...
func TestFullHTTPDefaultTraceResponsesMeetCommittedBudgetsAndFallbacksMatch(t *testing.T) {
	...
	assertNavigation := func(name, tool string, arguments map[string]any, collection, fallbackMarker string) {
		...
		if !strings.Contains(fallback, `"hasMore":true`) || !strings.Contains(fallback, `"continuation":"`) {
	...
	assertRange := func(name, tool string, arguments map[string]any) {
		...
		structured := result["structuredContent"].(map[string]any)["result"].(map[string]any)
		fallback := result["content"].([]any)[0].(map[string]any)["text"].(string)
		var fallbackFacts map[string]any
		if err := json.Unmarshal([]byte(fallback), &fallbackFacts); err != nil || result["isError"] == true || !reflect.DeepEqual(fallbackFacts, structured) {
	...
	want := map[string]int{
		"inventory":           12001,
		"compact frames":      28789,
		"detailed frames":     27523,
		"record descriptors":  29055,
		"inline records":      18241,
		"search":              29133,
		"semantic text range": 30355,
		"raw base64 range":    27307,
	}
