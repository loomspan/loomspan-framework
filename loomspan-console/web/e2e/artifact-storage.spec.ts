...
function makeLargeChunkedPayloadArtifact(): Buffer {
  ...
  const records: Array<Record<string, unknown>> = [
    { ...common, sequence: 1, recordType: "TRACE_STARTED", metadata: { tracePath: "generated/large-chunked-payload.ndjson", consoleCompatibilityVersion: "1.0.0-beta.6-SNAPSHOT", generationId: traceId }, data: { sessionId } },
  ...
  targetApp: async ({}, use) => {
    ...
    const server = makeTargetServer({
      ...
      artifactBodies: {
        ...
        "trace-canonical-concurrent-contract": readFixture("canonical-concurrent-contract.ndjson"),
      },
    });
...
test("WF-PLAN-NAV plan records open reloadable authoritative plan evidence", async ({ page, consoleProcess, targetApp }) => {
  await connectToTarget(page, consoleProcess, targetApp.origin);
  await acquireAndOpenExplorer(page, consoleProcess, "trace-canonical-concurrent-contract");
  await page.getByRole("tab", { name: "Records" }).click();

  const updateAction = page.getByRole("button", { name: "View plan change concurrent-plan from record 7" });
  await updateAction.focus();
  await page.keyboard.press("Enter");
  await expect(page).toHaveURL(/view=plans/);
  await expect(page).toHaveURL(/planId=concurrent-plan/);
  await expect(page).toHaveURL(/transitionSequence=7/);
  const transition = page.locator('.finalized-plan-transition[aria-current="true"]');
  await expect(transition).toBeFocused();
  await expect(transition).toContainText("Task D (task-d)");
  await expect(transition).toContainText("Pending → In progress");

  await page.reload();
  await expect(page.locator('.finalized-plan-transition[aria-current="true"]')).toBeFocused({ timeout: 15_000 });
  await expect(page).toHaveURL(/transitionSequence=7/);
  await expectNoSeriousAccessibilityViolations(page);

  await page.setViewportSize({ width: 640, height: 720 });
  await page.emulateMedia({ forcedColors: "active", reducedMotion: "reduce" });
  await expect(page.locator('.finalized-plan[aria-current="true"]')).toHaveCSS("outline-style", "solid");
  await expect(page.locator('.finalized-plan-transition[aria-current="true"]')).toHaveCSS("outline-style", "solid");
});

test("WF-EXPENSIVE-EXECUTION explores returned expandable timeline and frame usage", async ({ page, consoleProcess, targetApp }) => {
  await connectToTarget(page, consoleProcess, targetApp.origin);
  await acquireAndOpenExplorer(page, consoleProcess, "trace-nested-frame-usage");
  const root = page.locator('button[data-frame="root"]');
  const child = page.locator('button[data-frame="skill"]');
  await root.focus();
  ...
  const timelineHitTarget = page.locator('[data-frame-id="skill"]');
  const timelineBar = timelineHitTarget.locator("xpath=../*[name()='svg']/*[contains(@class, 'trace-timeline-bar')]");
  const barFill = await timelineBar.evaluate((element) => getComputedStyle(element).fill);
  await timelineHitTarget.hover();
  await expect(page.getByRole("tooltip")).toContainText("Duration:");
  await expect(timelineHitTarget).toHaveCSS("cursor", "default");
  await expect(timelineBar).toHaveCSS("fill", barFill);
  await expect(page).toHaveURL(/frameId=skill/);
...
test("WF-UNFAMILIAR-SKILL-PATH retains repeated invocation selection across views and reload", async ({ page, consoleProcess, targetApp }) => {
  ...
  const repeated = page.locator('button[data-frame="skill-1"], button[data-frame="skill-2"]');
