...
export function MCPIntegration() {
  ...
  return (
    <section aria-labelledby="mcp-title" className="overview-card">
      ...
      {!status && !error && <p className="observability-note">Loading MCP status…</p>}
      {status && <>
        <dl className="status-grid">
          <div><dt>State</dt><dd><span className={`status-chip mcp-state-${status.state.toLowerCase().replaceAll("_", "-")}`}>{status.state === "DISABLED_INVALID" ? "Disabled — invalid key file" : status.state === "ENABLED" ? "Enabled" : "Disabled"}</span></dd></div>
          <div><dt>Endpoint</dt><dd><code>{status.endpoint}</code></dd></div>
        </dl>
        {status.diagnostic && <p className="activity-notice" role="status">{status.diagnostic}</p>}
        ...
        {credential && <div className="credential-reveal" role="status">
          <strong className="credential-reveal-label">Access key</strong>
          <code className="credential-reveal-value">{credential}</code>
        ...
        <section className="view-subsection" aria-labelledby="mcp-setup-title">
          <h3 id="mcp-setup-title">Client setup</h3>
          <p className="observability-note">Use user or global client settings. Never put this key in a repository, URL, or shell command.</p>
          <div className="mcp-setup-grid">{status.setup.map((entry) => <article className="mcp-setup-card" key={entry.client}><h4>{entry.client}</h4><p className="mcp-setup-scope"><strong>{entry.scope}</strong> scope</p><p className="mcp-setup-guidance">{entry.guidance}</p><code className="mcp-setup-header">Authorization: Bearer &lt;LOOMSPAN_MCP_ACCESS_KEY&gt;</code></article>)}</div>
        </section>
      </>}
    </section>
  );
...
function Confirm({ text, action, onConfirm, onCancel }: { text: string; action: string; onConfirm(): void; onCancel(): void }) {
  return <div className="mcp-confirm" role="alertdialog" aria-label={action}>
    <p>{text}</p>
    <div className="view-actions">
