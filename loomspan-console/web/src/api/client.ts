...
export function getTraceRecords(traceId: string, cursor?: string, filter: TraceRecordFilter = {}, source: TraceSource = "TARGET"): Promise<TraceAnalysisPage<TraceRecord>> { return post("/api/console/v1/traces/analysis/records", { traceId, source, pageSize: 1000, cursor: cursor ?? "", filter, representation: "LOGICAL" }); }
export function getTracePlans(traceId: string, cursor?: string, planId?: string, source: TraceSource = "TARGET"): Promise<TraceAnalysisPage<PlanSummary>> { return post("/api/console/v1/traces/analysis/plans", { traceId, source, pageSize: 100, cursor: cursor ?? "", ...(planId ? { planId } : {}) }); }
...
export const getTraceAttempts = (traceId: string, cursor?: string, source: TraceSource = "TARGET") => traceFactPage<TraceAttempt>("attempts", traceId, cursor, source);
