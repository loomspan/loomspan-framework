...
test("trace plans request whole authoritative plan pages", async () => {
  const fetch = vi.fn().mockResolvedValue(
    new Response(JSON.stringify({ source: "TARGET", targetScopeId: "scope-1", items: [], hasMore: false, nextCursor: null }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
  ...
  vi.stubGlobal("fetch", fetch);
  await getTracePlans("trace-1", "cursor-1", "plan-1", "TARGET");
  expect(fetch).toHaveBeenCalledWith("/api/console/v1/traces/analysis/plans", expect.anything());
  const body = JSON.parse((fetch.mock.calls[0]?.[1] as RequestInit).body as string);
  expect(body).toEqual({ traceId: "trace-1", source: "TARGET", pageSize: 100, cursor: "cursor-1", planId: "plan-1" });
});

test("get skill detail sends registeredName in body", async () => {
  const fetch = vi.fn().mockResolvedValue(
    new Response(JSON.stringify({ registeredName: "MySkill", source: "YAML", sourcePath: "/path", yaml: "key: value" }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    }),
  );
