...
export type ActiveBranch = {
  ...
  effectiveConcurrency: boolean | null;
  path: FramePathEntry[];
};
...
export type TraceFrame = {
  ...
  planId?: string | null; taskId?: string | null; stepNumber?: number | null;
  parallelGroup?: string | null; effectiveConcurrency?: boolean | null;
};
...
export type PlanUpdate =
  | { previousSequence: number; availability: "AVAILABLE"; changes: PlanFieldChange[] }
  | { previousSequence: number; availability: "LIMIT_EXCEEDED" };

export type PlanReference = { planId: string; capabilityName: string };
export type PlanTask = {
  stepNumber: number; taskId: string; title: string; status: string;
  capabilityName: string | null; intent: string | null; dependsOn: string[];
  expectedOutputs: string[]; parallelGroup: string | null; note: string | null;
  assignedFrameId: string | null; effectiveConcurrency: boolean | null;
  failureIds: string[];
};
export type PlanExecutionUnit = {
  position: number; parallelGroup: string | null; taskIds: string[];
  effectiveConcurrency: boolean | null; observedOverlap: boolean | null;
};
export type PlanTransition = {
  sequence: number; kind: string; taskIds: string[]; parallelGroup: string | null;
  effectiveConcurrency: boolean | null; outcome: string;
};
export type PlanSummary = {
  planId: string; capabilityName: string; createdAt: string; status: string;
  creationSequence: number; traceRootFrameId: string; missionFrameId: string;
  planningFrameId: string; attemptId: string; retrySequenceId: string;
  tasks: PlanTask[]; executionUnits: PlanExecutionUnit[]; transitions: PlanTransition[];
};
...
export type TraceAttemptDiagnosticDescriptor = { kind: string; contentType: string; text: string; truncated: boolean; captureLimitBytes: number };
export type TraceAttemptDiagnosticPayload = { diagnostics: TraceAttemptDiagnosticDescriptor[] };
