...
function UsageRows({ rows }: { rows: Array<[string, TraceUsageValue | undefined, boolean | undefined]> }) {
  return <tbody>{rows.map(([kind, value, complete]) => <tr key={kind}><th scope="row">{kind}{complete === false && <span> (incomplete)</span>}</th><td className="trace-numeric">{value?.promptUnits ?? "unavailable"}</td><td className="trace-numeric">{value?.completionUnits ?? "unavailable"}</td><td className="trace-numeric">{value?.totalUnits ?? "unavailable"}</td></tr>)}</tbody>;
}
...
function LimitRow({ label, value, limit }: { label: string; value: number; limit: number }) {
  return <tr><th scope="row">{label}</th><td className="trace-numeric">{value}</td><td className="trace-numeric">{limit}</td><td className="trace-numeric">{limit === 0 ? "undefined" : percentage(value, limit)}</td></tr>;
}
...
function Limits({ limits, summary }: { limits: ConfiguredLimits | null; summary: TraceAnalysisSummary }) {
  ...
  return <div className="observability-table-region" role="region" aria-label="Configured limit comparison" tabIndex={0}><table>
    ...
    <thead><tr><th>Counter</th><th className="trace-numeric">Observed</th><th className="trace-numeric">Configured limit</th><th className="trace-numeric">Proportion</th></tr></thead>
    <tbody>
