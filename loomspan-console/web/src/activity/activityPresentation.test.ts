...
describe("presentActivity", () => {
  ...
  it("classifies activity kinds by normal warning and error severity", () => {
    expect(presentActivity(makeActivity("MODEL_ATTEMPT_FAILED")).severity).toBe("warning");
    expect(presentActivity(makeActivity("ERROR_RECORDED")).severity).toBe("error");
    expect(presentActivity(makeActivity("STEP_ACTION_REJECTED")).severity).toBe("error");
    expect(presentActivity(makeActivity("PLAN_VALIDATION_FAILED")).severity).toBe("error");
    expect(presentActivity(makeActivity("TOOL_CALL_FAILED")).severity).toBe("error");
    expect(presentActivity(makeActivity("STEP_FAILED")).severity).toBe("error");
    expect(presentActivity(makeActivity("STEP_COMPLETED")).severity).toBe("normal");
  });
  ...
  it("leads an error with its message and shows the simple exception name with the qualified name retained", () => {
    const activity = makeActivity("ERROR_RECORDED", {
      ...
      exceptionType: "ai.loomspan.LoomspanToolException",
    });
    const p = presentActivity(activity);
    ...
    expect(p.facts).toContainEqual({
      ...
      title: "ai.loomspan.LoomspanToolException",
    });
  });
...
describe("formatElapsed", () => {
  ...
  it("formats multi-minute elapsed time", () => {
    expect(formatElapsed("2026-07-25T12:00:00Z", "2026-07-25T12:02:30Z")).toBe("2m 30.0s");
  });
