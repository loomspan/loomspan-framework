...
const WARNING_KINDS: ReadonlySet<ActivityKind> = new Set([
...
export function presentActivity(activity: Activity): ActivityPresentation {
  ...
  const severity: ActivityPresentation["severity"] = ERROR_KINDS.has(kind)
    ? "error"
...
function retryDelay(details: Record<string, unknown> | null): string | null {
  ...
  const formatted = formatDuration(millis);
