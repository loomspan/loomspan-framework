...
describe("ActivityNarrative", () => {
  ...
  it("preserves following when stream goes offline", () => {
    const { rerender } = render(
  ...
  it("does not mistake a programmatic scroll event for user intent", () => {
    render(<ActivityNarrative activities={[makeActivity("1", "TRACE_STARTED", "Started")]} isLive={true} />);
    const list = screen.getByRole("log");
    Object.defineProperty(list, "scrollHeight", { value: 1000, configurable: true });
    Object.defineProperty(list, "clientHeight", { value: 200, configurable: true });
  ...
  it("pauses when the user drags the scroll bar away from the bottom", () => {
    render(<ActivityNarrative activities={[makeActivity("1", "TRACE_STARTED", "Started")]} isLive={true} />);
    const list = screen.getByRole("log");
    Object.defineProperty(list, "scrollHeight", { value: 1000, configurable: true });
    Object.defineProperty(list, "clientHeight", { value: 200, configurable: true });
  ...
  it("renders warning and error activities with distinct accessible presentation", () => {
    render(<ActivityNarrative activities={[
      makeActivity("1", "MODEL_ATTEMPT_FAILED", "Provider attempt failed"),
      makeActivity("2", "ERROR_RECORDED", "Execution error recorded"),
      makeActivity("3", "STEP_COMPLETED", "Step completed"),
    ]} isLive={true} />);

    expect(screen.getByRole("listitem", { name: "Warning activity: Provider attempt failed" })).toHaveClass("warning");
    expect(screen.getByRole("listitem", { name: "Error activity: Execution error recorded" })).toHaveClass("error");
    expect(screen.getByRole("listitem", { name: "Normal activity: Step completed" })).toHaveClass("normal");
  });
