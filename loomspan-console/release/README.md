...
The client discovers the exact thirteen installed MCP tools with ordinary tool
discovery; `LOOMSPAN_get_runtime` reports current runtime/target status and is
not a capability inventory. Without the raw-read tool parsed debugging
continues, while exact storage/parser/projector forensics is unavailable. A
missing tool is not protocol or target incompatibility. Without MCP the skill
can provide only general practice; MCP remains independently usable without the skill. Hosted clients
