...
class SuccessfulSkillCompletionBoundaryTest
{
    ...
    void linkedCallIsCreditedOnlyThroughVerifiedPlanCompletionAfterExecutionReturns()
    {
        ...
        LoomspanSession session = ai.loomspan.internal.core.TestLoomspanSessions.withId("linked", "test.entry", 3);
        when(planning.markToolStarted(eq(session), eq(capability))).thenReturn(Optional.of("task-1"));
        ...
        order.verify(planning).markToolStarted(eq(session), eq(capability));
        order.verify(router).execute(eq(capability), any(), eq(session), eq(null));
        order.verify(planning).markToolCompleted(session, "task-1", capability.name());
        verify(state, never()).recordSuccessfulSkill(any(), any(), any(Boolean.class));
    }
    ...
    void failedAndCancelledUnplannedCallsNeverReceiveSuccessfulCredit()
    {
                ...
                new CancellationException("cancelled")))
        {
            ...
            assertThatThrownBy(() -> ai.loomspan.internal.core.TestExecutionBindings.callWithSession(
                    session, () -> callback.invoke(Map.of(), null))).isSameAs(failure);
            verify(state, never()).recordSuccessfulSkill(eq(capability.name()), eq(null), eq(true));
            verify(planning, never()).markToolCompleted(any(), any(), any());
        }
    }
    ...
    private static ExecutionPlan linkedPlan()
    {
        return new ExecutionPlan(
                ...
                List.of(new PlanTask(
                        ...
                        "Investigate", List.of(), List.of(), null, null)));
    }

    private static ExecutionFrame frame(String route)
    {
        return new ExecutionFrame(
                "frame-1", null, ai.loomspan.internal.core.OperationType.SKILL,
