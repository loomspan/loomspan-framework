...
class ObservabilityDtoMapperTest
{
    ...
    void activeElapsedIsDerivedAtObservationTimeAndNeverCreatesHealthClaims()
    {
        ...
        assertThat(dto.activeBranches()).singleElement().satisfies(branch ->
        {
            assertThat(branch.path()).hasSize(2);
            assertThat(branch.planId()).isNull();
        });
        assertThat(dto.status()).isEqualTo("ACTIVE");
        assertThat(dto.usage().getClass().getName())
                .isEqualTo("ai.loomspan.internal.observability.web.dto.ObservabilityDtos$Usage");
