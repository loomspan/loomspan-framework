...
| Terminal provider failure | `terminal-provider-failure-actionable` | Canonical terminal failure linked to its validated final failed attempt, whose payload contains stack, Loomspan guidance, and raw provider diagnostics in that order |
...
`ERROR_RECORDED` data. Provider-attempt fixtures instead carry bounded Java
stack, framework guidance, and provider diagnostic objects in the failed
attempt's ordinary logical payload. A terminal provider failure links the
canonical failure summary to that validated final-attempt payload. Terminality is derived only from a matching terminal
