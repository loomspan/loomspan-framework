...
- `SampleEvidenceContractCatalogTest#loadsTravelSchemasWithRequiredNullableTransportLegs` protects an actual pair of catalog-loaded contracts whose required transport leg keys may honestly be null when unavailable while keeping sample application code on the supported API.
