...
class ExecutionJournalProjectionContractTest {
    ...
    void projectsCanonicalDeveloperFacingJournalFromRepresentativeTraceStream() {
        ...
        OutputSchemaOutcome outputSchemaOutcome = new OutputSchemaOutcome(
                ...
                List.of(new OutputSchemaValidationIssue(
                        "$", "Response is not valid JSON.", "response",
                        "invalid_json", "valid JSON", "unparseable JSON",
                        "Unexpected character", 1, 4, 3L, "\"bad\"")));
        ...
        assertThat(entries.get(4).payload().get("issues").get(0).get("path").textValue()).isEqualTo("$");
        assertThat(entries.get(4).payload().get("issues").get(0).get("code").textValue()).isEqualTo("invalid_json");
        assertThat(entries.get(4).payload().get("issues").get(0).get("reason").textValue()).isEqualTo("Unexpected character");
        assertThat(entries.get(4).payload().get("issues").get(0).get("characterOffset").longValue()).isEqualTo(3L);
        assertThat(entries.get(4).payload().get("issues").get(0).get("fragment").textValue()).isEqualTo("\"bad\"");
    ...
    @Test
    void keepsFailedAttemptWarningInTraceOrderWithoutInferringAnError()
    {
        List<JournalEntry> entries = projector.project(List.of(
                record(1, TraceRecordType.MODEL_ATTEMPT_FAILED, "model-frame", "rootVisibleSkill#model",
                        Map.of(
                                "attemptId", "attempt-1",
                                ...
                                "retryDecision", "RETRY",
                                "retryDelayMillis", 0),
                        Map.of("diagnostics", List.of(Map.of("text", "do not project me")))),
                record(2, TraceRecordType.MODEL_RESPONSE_RECEIVED, "model-frame", "rootVisibleSkill#model",
                        Map.of("attemptId", "attempt-2", "retrySequenceId", "retry-1"),
                        Map.of("content", "recovered"))))
                .getEntriesSnapshot();
        ...
        assertThat(entries).singleElement().satisfies(entry ->
        {
            ...
            assertThat(entry.payload().toString()).doesNotContain("do not project me", "diagnostics");
        });
    }
