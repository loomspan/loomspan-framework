...
class ObservabilitySampleIntegrationTest
{
    ...
    @Test
    @Timeout(30)
    void javaSkillProducesAuthenticatedCatalogedDownloadThroughHostPermitAll() throws Exception
    {
        ...
        HttpResponse<byte[]> skill = send(ROOT + "/skills/expenseLookup", KEY, "application/json");
        assertThat(skill.statusCode()).isEqualTo(200);
        JsonNode source = json.readTree(skill.body());
        assertThat(source.path("source").asText()).isEqualTo("JAVA");
        assertThat(source.path("beanName").asText()).isEqualTo("expenseService");
        assertThat(source.path("method").asText()).contains("getLatestExpenses");
        assertThat(source.has("sourcePath")).isFalse();
        assertThat(source.has("yaml")).isFalse();
