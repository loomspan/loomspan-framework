...
class SampleApplicationTests {
    ...
    @Test
    void invokesAnnotationOnlyJavaSkillThroughSupportedFacade() {
    ...
    @Test
    void invokesSearchFlightsWithoutACompanionManifest()
    {
        assertThat(new ClassPathResource("skills/travel/search_flights.yml").exists()).isFalse();
        assertThat(skillTemplate.invoke("searchFlights", Map.of("scenario", "budget-nyc-weekend")))
                .contains("BudgetAir").contains("Metro Hopper");
    }

    @Test
    void expensesRetainExplicitNameAndProduceCompletedObservation()
    {
        var observation = new java.util.concurrent.atomic.AtomicReference<ai.loomspan.api.SkillExecutionView>();
        assertThat(new ClassPathResource("skills/basics/expense_lookup.yml").exists()).isFalse();
        skillTemplate.invoke("expenseLookup", Map.of(), observation::set);
        assertThat(observation.get()).isNotNull();
        assertThat(observation.get().events()).isNotEmpty();
    }
