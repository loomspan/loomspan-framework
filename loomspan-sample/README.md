...
A runnable Spring Boot 4 application that demonstrates Loomspan skills end to end: shared Java/YAML skill discovery, annotation-defined Java `@SkillMethod` skills with application-owned `@SkillParam` contracts, named-connection Spring AI 2 model routing, HTN planning, vision/attachment inputs, and HTTP invocation via `SkillTemplate`.
...
| Pattern | Where |
| --- | --- |
| Annotation-defined Java skill | `expenseLookup`, `weightTicketParser`, incident probes, insurance leaves, support CRM leaves, travel catalog leaves |
...
| Pure YAML vision skill with `attachment` input | `weightTicketParserBySkill` |
...
| Current execution events / session id via `SkillTemplate` observer | invoice, weight-ticket, incident, claims, support, and travel endpoints |
...
- **OpenAI API key** for weight-ticket vision demos:
...
After invoking the model-free Java endpoint
...
└── src/
    ...
    │   │   ├── SampleController.java               # Invoice/weight-ticket HTTP demos
    │   │   ├── ExpenseService.java                 # @SkillMethod (deterministic data)
    │   │   ├── WeightTicketExtractionService.java  # @SkillMethod (OpenAI vision HTTP)
    │   │   ├── incident/
    ...
    │           ├── vision/                         # Weight-ticket Java + pure YAML vision
...
Named Loomspan models (each LLM-backed YAML skill must set `model` to one of these keys; Java skills do not require one):
...
On-ramp patterns: annotation-defined Java, single-shot LLM, shallow planning.

#### 1. `expenseLookup` → Java

- **Source:** `ExpenseService.getLatestExpenses`
- **Type:** Java skill (`@SkillMethod(name = "expenseLookup")`)
- **Execution:** Model-free deterministic Java with a reflected input contract.
- **Behavior:** Returns a fixed list of fake expenses from `ExpenseService`.

Demonstrates: an explicit `@SkillMethod(name = "expenseLookup")` registers the Spring method directly. The controller and `allowed_skills` use `expenseLookup`. `SampleApplication` enables JSR-250; this method uses `@PermitAll`, preserving unrestricted sample access.
...
- **Type:** Planning skill (`planning_mode: true`, `concurrency: true`, `max_steps: 10`)
- **Model:** `granite4-tiny`
...
Demonstrates: multi-step agentic workflow — parse invoice, look up expenses, decide duplicate — with evidence constraints. Planning concurrency is default-enabled and explicitly shown here. The coordinator assigns exact tasks in accepted task-list order. Consecutive equal non-null `parallelGroup` values form a validated unit: with effective concurrency enabled, every member is admitted before dispatch, workers may overlap, the coordinator waits for all ordinary outcomes, and then folds results and diagnostics in task-list order. Ungrouped tasks and `concurrency: false` groups remain serialized. Group metadata records eligibility; only trace frame intervals prove observed overlap. Explicit `dependsOn` edges may reference only earlier execution units. Each assigned task costs one `max_steps` slot and final synthesis costs one more, so a whole unit is rejected before admission when it and the final step cannot fit.

### Vision (`skills/vision/`)

Document/image parsing with OpenAI (Java-backed vs pure YAML).

#### 4. `weightTicketParser` → Java + OpenAI vision

- **Source:** `WeightTicketExtractionService.extractSampleWeightTicket`
- **Type:** Java skill → `WeightTicketExtractionService.extractSampleWeightTicket`
- **Behavior:** Loads bundled `forms/weight_ticket.jpg`, calls OpenAI Responses API with a strict JSON schema, and returns ticket/time, scale/location, and certified axle/gross weight fields.
...
#### 5. `weightTicketParserBySkill` → pure YAML vision

- **File:** `skills/vision/weight_ticket_parser_by_skill.yml`
...
| Java leaves | `@SkillMethod` declares each skill; Java owns inputs and results |
...
| --- | --- | --- | --- |
| `GET` | `/expenses` | `expenseLookup` | Invokes the annotation-defined Java skill directly |
| `GET` | `/weight-ticket/parse-sample` | `weightTicketParser` | Uses bundled sample image inside the Java skill |
| `GET` | `/weight-ticket/parse-sample-by-skill` | `weightTicketParserBySkill` | Passes `classpath:/forms/weight_ticket.jpg` as an attachment |
...
`/expenses` calls `skillTemplate.invoke("expenseLookup", ...)`. The annotation explicitly names this skill; the method and bean locations are diagnostics, not aliases.
...
Invoke-RestMethod http://localhost:8081/weight-ticket/parse-sample

# Vision: pure YAML skill + attachment
Invoke-RestMethod http://localhost:8081/weight-ticket/parse-sample-by-skill
...
| Asset | Purpose |
| --- | --- |
| `src/main/resources/forms/weight_ticket.jpg` | Certified truck scale ticket image for vision demos |
...
1. Spring Boot starts `SampleApplication`; Loomspan completes Java and YAML registration in one exact-name registry and validates every child reference.
2. `SampleController` injects `SkillTemplate`.
3. Controllers call `skillTemplate.invoke(registeredName, inputs)` or the overload with a `Consumer<SkillExecutionView>` observer. The exact Java annotation/default name or YAML manifest name is the callable identity; bean/method locations are diagnostics.
4. Loomspan resolves the skill, then either selects its model alias and named connection for LLM-backed execution or invokes an annotation-defined Java skill through its Spring bean, and returns text (plus optional current-version execution events).
...
| Test class | Coverage |
| --- | --- |
| `SampleApplicationTests` | Context loads and annotation-only Java invocation through the supported `SkillTemplate` facade |
| `SupportedApiUsageArchitectureTest` | Sample production code depends only on `ai.loomspan.api` |
...
| `SampleControllerTest` | Controller delegates with registered skill names for expenses, weight tickets, and duplicate-invoice paths |
...
Normal test runs use Java skills or local protocol stubs and make no provider calls. To run the explicit OpenAI smoke test through the supported facade, set `OPENAI_API_KEY` and run from the repository root:
...
| Weight-ticket endpoints fail with missing API key | `OPENAI_API_KEY` not set in the process environment |
